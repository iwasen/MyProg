...
// ジョブ情報取得
$sql = "SELECT jb_guide_subject,jb_guide_from_name,jb_guide_content,jb_guide_send_date,jb_guide_add_mail"
		. " FROM t_job"
...
if (pg_num_rows($result) > 0) {
	...
	// 送信を年月日時分に分解
	if ($fetch->jb_guide_send_date) {
		$send_date = sql_time($fetch->jb_guide_send_date);
...
function onsubmit_form1(f) {
	...
	switch (f.proc.value) {
	case "preview":
		f.action = "guidemail_preview.php";
		f.target = "_blank";
		return true;
	case "testmail":
		f.action = "guidemail_testmail.php";
		f.target = "_self";
		return confirm("テストメールを送信します。よろしいですか？");
	case "update":
		f.action = "guidemail_update_comp.php";
		f.target = "_self";
		return confirm("案内メールを更新します。よろしいですか？");
	}
	return false;
}
//-->
...
<!-- main -->
<form method="post" name="form1" action="guidemail_update_comp.php" onsubmit="return onsubmit_form1(this)">
<input type="hidden" name="job_id" <?=value($job_id)?>>
...
<tr>
	...
	<td width="760">
		...
		<tr>
			...
			<td width="560"><input type="text" name="subject" <?=value($fetch->jb_guide_subject)?> size="60" class="form"></td>
		</tr>
		<tr>
			<th>差出人名称</th>
			<td><input type="text" name="from_name" <?=value($fetch->jb_guide_from_name)?> size="30" class="form"></td>
		</tr>
		<tr>
			<th>本文</th>
			<td><textarea name="content" cols="80" rows="30" class="form"><?=htmlspecialchars($fetch->jb_guide_content)?></textarea></td>
		</tr>
		<tr>
			<th>送信日</th>
