...
// 使用不可変数チェック
$err_var_ary = check_personal_var($_POST['content']);
if ($err_var_ary) {
	foreach ($err_var_ary as $var_name)
		$msg[] = "開始メールに使用できない差し込み変数 $var_name が含まれています。";
}
...
if (!$msg) {
	// ジョブ更新
	$rec['jb_start_subject'] = sql_char($_POST['subject']);
	$rec['jb_start_from_name'] = sql_char($_POST['from_name']);
	$rec['jb_start_content'] = sql_char($_POST['content']);
	$rec['jb_start_send_date'] = sql_date("{$_POST['send_year']}/{$_POST['send_month']}/{$_POST['send_day']} {$_POST['send_hour']}:{$_POST['send_minute']}");
	$rec['jb_start_add_mail'] = sql_char($add_mail);
	$rec['jb_start_status'] = sql_number(1);
