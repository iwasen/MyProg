...
function get_nickname_ary($job_id) {
	...
	if ($_SESSION[SCD]['room_type'] == ROOM_PIR) {
		$where = sprintf("jm_job_id=%s AND jm_status=1 AND (jm_member_id=%s OR " . cond_participant_auth('司会者') . ")",
				sql_number($job_id),
...
function get_record_ary($job_id, $remark_id) {
	...
	if ($remark_id_ary) {
		$sql = "SELECT rm_member_id,rm_seq_no,rm_subject,rm_content"
				. " FROM l_remark"
		...
		for ($i = 0; $i < $nrow; $i++) {
			...
			$remark_ary[$row][$member_id]->seq_no = $fetch->rm_seq_no;
			$remark_ary[$row][$member_id]->subject = $fetch->rm_subject;
			$remark_ary[$row][$member_id]->content = $fetch->rm_content;
		}
	}
