...
function check2(f) {
	...
	if (f.from_no.value != "" && f.to_no.value != "") {
		if (Number(f.from_no.value) > Number(f.to_no.value)) {
			alert("終了発言番号は開始発言番号より大きい番号を入力してください。");
			f.to_no.focus();
			return false;
		}
	}
...
function check3(f) {
	if (f.no.value.match(/[0-9\-,]+/g) != f.no.value) {
		alert("発言番号の指定に数字、ハイフン、カンマ以外の文字は入力できません。");
		f.no.focus();
		return false;
	}
	return true;
...
<tr>
	<td>
		...
		<tr>
			<td class="text12b">今までの発言のログをダウンロード</td>
		</tr>
		</table></td>
</tr>
</table>
	...
	<td>
		...
		<tr>
			...
			<tr>
				...
				<tr>
					<td colspan="3" class="text14b">詳細を指定して発言をダウンロードできます。</td>
				</tr>
				<tr>
					<td colspan="3"><hr size="1" width="100%" align="center" color="#c0c0c0" noshade></td>
				</tr>
				<tr>
					<td colspan="3" class="text14">期間を指定して発言をダウンロードする。</td>
				</tr>
				<tr>
					<td width="25">&nbsp;</td>
				...
				<tr>
					<td colspan="3" class="text14">発言番号を<strong>連番で</strong>指定して発言をダウンロードする。<br>
													※発言番号とは発言のサブジェクトの最初についている[0000]のような番号のことです。</td>
				</tr>
				<tr>
					<td width="25">&nbsp;</td>
					<td colspan="2">1〜5と入力した場合、1・2・3・4・5の発言全てがダウンロードされます。<br>
									1・3・5のみをダウンロードする場合は、
									一番下の「発言番号を別々に指定して発言をダウンロードする」から指定してください。</td>
				</tr>
				<tr>
					<td width="25">&nbsp;</td>
				...
				<tr>
					<td colspan="3" class="text14">発言番号を<strong>別々に</strong>指定して発言をダウンロードする。<br>
													※発言番号とは発言のサブジェクトの最初についている[0000]のような番号のことです。</td>
				</tr>
				<tr>
					<td width="25">&nbsp;</td>
					<td colspan="2">【1,3,5】のように「半角カンマ」で区切って入力してください。<br>
									【1,3,5-10】とハイフンでつなげることにより、1・3・5・6・7・8・9・10の発言をダウンロードできます。</td>
				</tr>
				<tr>
					<td width="25">&nbsp;</td>
