...
$sql = "SELECT cm_seq_no,cm_subject,cm_body,cm_send_date,cm_status" .
			" FROM t_close_mail WHERE cm_seq_no=$cm_id";
$result = db_exec($sql);
if (pg_numrows($result) == 0)
	redirect('list.php');
$fetch = pg_fetch_object($result, 0);

if ($fetch->cm_send_date) {
	$send_date_check = '1';
	$send_date = sql_time($fetch->cm_send_date);
} else
...
function onSubmit_form1(f) {
	...
	case "update":
		return input_check(f, "完了督促メールを更新します。よろしいですか？");
	case "delete":
		return confirm("完了督促メールを削除します。よろしいですか？");
	case "test":
...
<table border=0 cellspacing=2 cellpadding=3 width="80%">
	...
	<tr>
		...
		<td class="n1">
			<input class="kanji" type="text" name="subject" size=80 maxlength=100 <?=value($fetch->cm_subject)?>>
	...
	<tr>
		...
		<td class="n1">
			...
			<textarea class="kanji" name="body" cols=78 rows=20><?=htmlspecialchars($fetch->cm_body)?></textarea><br>
