...
if (!$next_action) {
	$sql = "SELECT * from t_admin_msg where an_seq_no=$an_no";
	$result = db_exec($sql);
	if (pg_numrows($result) == 0)
		system_error('メッセージＩＤが不正', __FILE__);
	$fetch = pg_fetch_object($result, 0);
	$start_date = $fetch->an_start_date;
	$end_date = $fetch->an_end_date;
	$title = $fetch->an_title;
	$msg = $fetch->an_text;
} else {
...
function update_check(f) {
	...
	if (confirm("事務局からのお知らせを更新します。よろしいですか？")) {
		f.action = "update.php";
		f.submit();
	}
}
function delete_check(f) {
	if (confirm("事務局からのお知らせを削除します。よろしいですか？")) {
		f.action = "update.php";
		f.submit();
	}
}
...
<br>
<input type="hidden" name="an_no" <?=value($an_no)?>>
