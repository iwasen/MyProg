...
// ソート条件
$order_by = order_by(6, 1, 'mb_id', 'mb_name1_kana || mb_name2_kana', 'mb_mail_addr', 'mb_zip', 'mb_area_cd', 'li_delivery_date');

// 表示行数条件
$limit = disp_limit();

$sql = "select mb_seq_no,mb_id,mb_name1,mb_name2,mb_name1_kana,mb_name2_kana,mb_mail_addr,mb_zip,mb_area_cd,mb_address,li_delivery_date"
	. " from t_member left join t_present_list on mb_seq_no=li_mb_seq_no where li_status=5 and li_ps_no=$ps_no $order_by";
$result = db_exec($sql);
...
<table border=0 cellspacing=0 cellpadding=1 width="100%">
	<tr>
		<td class="lt">■発送手配完了一覧　<font size=-1 color="blue">（総数:&nbsp;<?=$nrow?>件）</td>
		<td class="lb">
...
?>
	<tr class="tc<?=$i % 2?>">
		...
		<td align="center"><?=format_date($fetch->li_delivery_date)?></td>
