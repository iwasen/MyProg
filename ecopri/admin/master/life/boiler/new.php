...
function onSubmit_form1(f) {
	...
	if (f.name.value == "") {
		alert("給湯器タイプ名を入力してください。");
		f.name.focus();
		return false;
	}
	return confirm("給湯器タイプ名を登録します。よろしいですか？");
}
//-->
</script>
</head>
<body onload="document.form1.bt_order.focus()">
<? page_header() ?>
...
<table border=0 cellspacing=2 cellpadding=3 width="100%">
	<tr>
		<td class="m0" colspan=2>■登録する給湯器タイプ情報を入力してください。</td>
	</tr>
	<tr>
		<td class="m1" width="20%">表示順序<?=MUST_ITEM?></td>
		<td class="n1">
			<input class="number" name="bt_order" size=20 maxlength=4>
			<span class="note">（半角数字）</span>
		</td>
	</tr>
	<tr>
		<td class="m1">給湯器タイプ名<?=MUST_ITEM?></td>
		<td class="n1">
