...
$ol_use = explode(',', $reg->ol_use);
$ol_sum = explode(',', $reg->ol_sum);

// やっぱり入力する
if ($inp == 'on' && $reg->commit_flag == 0) {
	$sql = "UPDATE t_base_data SET bd_ol_inp_cd=null WHERE bd_seq_no=" . $reg->bd_seq_no;
	db_exec($sql);
}
...
<div align="center">
<form name="form1" method="post" action="result_ol_check.php">
<table width="714" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
	...
	<tr>
		<td align="center" colspan=3><img src="img/result_input/calendar/touyu_calender.gif"></td>
	</tr>
	<tr>
		<td align="center" colspan=3>
			<table border=0 cellspacing=0 cellpadding=0 bgcolor="#99cccc" width=90% height=470>
				<tr>
...
while ($d <= $ed) {
	...
	if (date('m', $d) == $month) {
		...
		$input1 = commit_check($ol_use[$i], 'ol_use', $reg->commit_flag, 8, 4) . "</td><td>L"; 
		$input2 = commit_check($ol_sum[$i], 'ol_sum', $reg->commit_flag, 8, 5) . "</td><td>円";
		$i++;
	} else {
		$color = 'gray';
		$bgcolor = '#FFFFFF';
...
?>
			<table width=90%>
				<tr>
					<td align="right"><a href="result_ol_q.php"><img src="img/result_input/calendar/no_input.gif" alt="今月は入力しない" border=0></a></td>
				</tr>
			</table>
<?
}
