...
if (count($read_chk) == 0) {
	...
	$mag_datas = "";
	$error_datas = "<br>　一度に登録・削除するメルマガ数は30以下にしてください";
	include("tmpl_html/mag-regi_thanks.html");
	exit();
} else {
...
//****************************************強制購読
			...
			if ( IsPortableAddr($email) ){
				$mag_reg_cnt = 1;	// 2001.08.22 携帯読者には送らない。
			}
			// --------------------------------//

//バ・キューン
			if ($mag_id != $bakyun_id && $mag_reg_cnt == 0) {  /* 要件NO.1 既存の読者の場合はさわらず */
				$mag_nm = "";
			...
			if ($mag_id != $letter_id && $mag_reg_cnt == 0) {  /* 要件NO.1 既存の読者の場合はさわらず */
			//めろんぱんレター
				$mag_nm = "";
			...
			if ($mag_id != $html_letter_id && $mag_reg_cnt == 0) {  /* 要件NO.1 既存の読者の場合はさわらず */
			//めろんぱんHTMLレター
				$mag_nm = "";
				...
				if ($ct > 0) { ;
					...
					if (($mail_id = update_squ_key ("T_MAILADDR")) != "-1") {
						...
						$sql .= "to_timestamp('$modify_dt','YY-MM-DD HH24:MI:SS'),0,to_timestamp('$modify_dt','YY-MM-DD HH24:MI:SS'))";
						$result = db_exec($sql);
						//mag_let_mail($email,$html_letter_id,"めろんぱんレター");
					}
				}
			}
