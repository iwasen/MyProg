...
<form method="post" action="letter_tsuushin3.php" name="form1" onSubmit="return OnSubmit_form1();">
  ...
  <table border=0 cellspacing=2 cellpadding=3 width="750">
    ...
    <tr>
      ...
      <td class="n6">
				<select name="send_year"><? select_year(2001, ' ', ''); ?></select>年
				<select name="send_month"><? select_month(' ', ''); ?></select>月
				<select name="send_day"><? select_day(' ', ''); ?></select>日
				<select name="send_hour"><? select_hour(' ', ''); ?></select>時
				<select name="send_minutes"><? select_minute(' ', ''); ?></select>分
			</td>
    </tr>
  </table>
