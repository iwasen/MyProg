...
if ($nrow == 0) {
	...
	if ($mag_pub_status_flg <> '01') {
		...
		if ($job_flg == "ok") {
			$sql = "UPDATE M_MAGAZINE SET MAG_PUB_STATUS_FLG = '03', MAG_AGR_DT = 'now' WHERE mag_id = '$mag_id'";
			db_exec($sql);
			mag_honok_mail($owner_nm_kanj, $mail_add_main, $mag_nm);
			$msg = "¾µÇ§¤·¤Ş¤·¤¿¡£";
		} elseif ($job_flg == "ng") {
			$sql = "UPDATE M_MAGAZINE SET MAG_PUB_STATUS_FLG = '02', MAG_AGR_DT = 'now' WHERE mag_id = '$mag_id'";
			db_exec($sql);
			mag_honng_mail($owner_nm_kanj, $mail_add_main, $mag_nm);
			$msg = "¾µÇ§¤òµñÈİ¤·¤Ş¤·¤¿¡£";
		}
	}
}
...
ËÜÅĞÏ¿¤ò<?= $msg ?><br>
<br>
<input type="button" value=" Ìá¤ë " onclick="location.href='hon_touroku1.php'">
</center>
</form>
