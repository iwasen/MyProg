...
$sql ="SELECT PUBLISHER_ID, MAIL_ADD_MAIN, OWNER_NM_KANJ, OWNER_NM_KANA, OWNER_ZIP1, OWNER_ZIP2, OWNER_PREFEC, OWNER_ADD, OWNER_TEL, OWNER_PSWD";
$sql = $sql . " FROM M_PUBLISHER";
$sql = $sql . " WHERE PUBLISHER_ID =" . sql_number($pub_id);
$result = db_exec($sql);
if (!pg_numrows($result)) {
		header ("location: ../admin/err_msg.php?id=1008");
	exit;
} else {
...
function OnSubmit_form1(publisher_id) {
...
<form method="post" name="form1" action="riba_regist5.php" onSubmit="return OnSubmit_form1();">
	<center>
		<table width="800">
			<tr>
				<td>■メルマガ発行者情報</td>
