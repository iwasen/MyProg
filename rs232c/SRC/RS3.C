...
/*	ワークエリア	*/
u_char	rs3_cmdreg;			/* コマンドレジスタセーブエリア */
...
static	int	(*rs3_rcvfunc)(int);	/* ユーザ受信割り込み関数ポインタ */
...
/*	内部関数	*/
static	int	rs3_prmcheck(RSPRM *, u_int *, u_int *);
static	void	interrupt far	rs3_int(void);
...
int	rs3_open(RSPRM *rsprm)
{
	...
	/* オープンパラメータチェック */
	if ((err = rs3_prmcheck(rsprm, &mode, &cmd)) != 0)
		return(err);
	...
	/* ＲＳ－２３２Ｃ割り込みマスク */
	outp2(IOP_RS3_IMR, imreg = 0);

	/* ８２５１Ａハードウェアリセット */
	outp2(IOP_RS3_MOD, 0);
	outp2(IOP_RS3_MOD, 0);
	outp2(IOP_RS3_MOD, 0);
	outp2(IOP_RS3_MOD, 0x40);

	/* モードレジスタセット */
	outp2(IOP_RS3_MOD, mode);

	/* 同期キャラクタセット */
	if (rsprm->sync_mode == SYNC) {
		outp2(IOP_RS3_SYN, rsprm->syn_chr);
		if ((mode & 0x80) == 0)
			outp2(IOP_RS3_SYN, rsprm->syn_chr >> 8);
	}

	/* コマンドセット */
	rs3_cmdreg = cmd;
	outp2(IOP_RS3_MOD, rs3_cmdreg | CMD_ER | CMD_EH);

	/* ポインタ＆カウンタ初期化 */
	read_ptr = write_ptr = rcv_count = 0;

	/* ベクタ番号取り出し */
	vect_no = vect_no_tbl[inp(IOP_RS3_SGN) & 0x03];

	/* 割り込みベクタ保存 */
	save_vect = _dos_getvect(vect_no);

	/* 割り込みベクタセット */
	_dos_setvect(vect_no, rs3_int);

	/* 割り込みマスクビット取り出し */
	imr_bit = imr_tbl[inp(IOP_RS3_SGN) & 0x03];
	ims_flg = ims_tbl[inp(IOP_RS3_SGN) & 0x03];
	...
	outp2(IOP_RS3_IMR, imreg = IMR_RXRE);
...
int	rs3_reopen(RSPRM *rsprm)
{
	...
	/* 同期キャラクタセット */
	if (sync1_flag) {
		(void)outp(IOP_RS3_SYN, sync1);
		if (sync2_flag)
			(void)outp(IOP_RS3_SYN, sync2);
	}

	/* コマンドセット */
	rs3_cmdreg = cmd;
	(void)outp(IOP_RS3_MOD, rs3_cmdreg | CMD_ER | CMD_EH);
...
/*=======================================================================
 ...
 |
 |	int	rs3_prmcheck(rsprm, mode, cmd)
 |
 |		RSPRM	*rsprm;		ＲＳ－２３２Ｃオープンパラメータ
 |		u_int	*mode;		８２５１Ａモード
 |		u_int	*cmd;		８２５１Ａコマンド
 ...
 |
 =======================================================================*/
static	int	rs3_prmcheck(RSPRM *rsprm, u_int *mode, u_int *cmd)
{
	/* モード、コマンド初期化 */
	*mode = 0;
	*cmd = 0;
...
int	rs3_get(void)
{
	...
	case FLOW_SOFT:		/* ＸＯＮ／ＸＯＦＦ制御 */
		if (flow_rflag != 0 && rcv_count < flow_on) {
			outp2(IOP_RS3_DAT, XON);	/* ＸＯＮ送信 */
			flow_rflag = 0;
		}
		break;
	case FLOW_HARD:		/* ＲＴＳ／ＣＴＳ制御 */
		if (flow_rflag != 0 && rcv_count < flow_on) {
			rs3_cmd(CMD_RTS, CMD_RTS);	/* ＲＴＳオン */
			flow_rflag = 0;
		}
		break;
	}
...
void	rs3_put(int c)
{
	...
	/* 送信可待 */
	while ((inp(IOP_RS3_STS) & STS_TXRDY) == 0)
		;
...
void	rs3_send(char *s, int len)
{
	...
	if (duplex == DUPLEX_HALF) {
		/* 送信可、受信禁止、ＲＴＳオン */
		rs3_cmd(CMD_TXE|CMD_RTS, CMD_TXE|CMD_RXE|CMD_RTS);
	}
	...
	if (duplex == DUPLEX_HALF) {
		rs3_cmd(0, CMD_TXE);	/* 送信禁止 */

		/* 送信終了を待つ */
		while ((inp(IOP_RS3_STS) & STS_TXE) == 0)
			;
		/* 受信可、ＲＴＳオフ */
		rs3_cmd(CMD_RXE, CMD_RXE|CMD_RTS);
	}
}
...
/*=======================================================================
 ...
 |
 =======================================================================*/
int	rs3_sendreq(char *s, int len)
{
	int	data_size;
	...
	if (duplex == DUPLEX_HALF){
		/* 送信可、受信禁止、ＲＴＳオン */
		rs3_cmd(CMD_TXE|CMD_RXE|CMD_RTS, CMD_TXE|CMD_RTS);
	}

	outp2(IOP_RS3_IMR, imreg |= IMR_TXRE); /* 送信可割り込み許可 */
...
/*=======================================================================
 ...
 |		ＰＡＤ文字設定
 |
 |	void	rs3_pad(lpad, lpadsize, tpad, tpadsize)
 |
 ...
 |
 =======================================================================*/
void	rs3_pad(char *lpad, int lpadsize, char *tpad, int tpadsize)
{
...
/*=======================================================================
 ...
 |		受信割り込み処理設定
 |
 |	void	rs3_rcvint(func)
 |
 |		int	(*func)();	受信割り込み処理
 |
 =======================================================================*/
void	rs3_rcvint(int (*func)(int))
{
	rs3_rcvfunc = func;
}
...
void	rs3_ehm(void)
{
	outp2(IOP_RS3_MOD, rs3_cmdreg | 0x80);
}
...
int	rs3_dsr(void)
{
	return(inp(IOP_RS3_STS) & STS_DSR ? 1 : 0);
}
...
int	rs3_cd(void)
{
	return(inp(IOP_RS3_SGN) & SGN_CD ? 0 : 1);
}
...
int	rs3_cts(void)
{
	return(inp(IOP_RS3_SGN) & SGN_CS ? 0 : 1);
}
...
int	rs3_ci(void)
{
	return(inp(IOP_RS3_SGN) & SGN_CI ? 0 : 1);
}
...
/*=======================================================================
 ...
 |		コマンド出力
 |
 |	void	rs3_cmd(cmd, mask)
 |
 |		int	cmd;		コマンド
 |		int	mask;		マスクデータ
 |
 =======================================================================*/
void	rs3_cmd(int cmd, int mask)
{
	_disable();
	rs3_cmdreg = (cmd & mask) | (rs3_cmdreg & ~mask);
	outp2(IOP_RS3_MOD, rs3_cmdreg);
	_enable();
}
...
static	void	interrupt far	rs3_int(void)
{
	...
	if (sts & STS_RXRDY) {
		...
		if (sts & 0x38) {
			/* エラーリセット */
			outp2(IOP_RS3_MOD, rs3_cmdreg | 0x10);
			c |= (sts & 0x38) << 8;
		}
		...
		if (c == -1)
			...
			if (send_len != 0) {
				/* 送信禁止 */
				outp2(IOP_RS3_MOD, rs3_cmdreg &= ~CMD_TXE);
			}
			...
			case FLOW_HARD:
				if (flow_rflag == 0 && rcv_count > flow_off) {
					/* ＲＴＳオフ */
					outp2(IOP_RS3_MOD, rs3_cmdreg &= ~CMD_RTS);
					flow_rflag = 1;
				}
				break;
			}
	...
	if ((imreg & IMR_TXEE) && (sts & STS_TXE)) {
		/* ＥＭＰＴＹ割り込み禁止 */
		outp2(IOP_RS3_IMR, imreg &= ~IMR_TXEE);

		if (duplex == DUPLEX_HALF) {
			/* 受信可、ＲＴＳオフ */
			rs3_cmdreg = rs3_cmdreg & ~(CMD_RXE|CMD_RTS) | CMD_RXE;
			outp2(IOP_RS3_MOD, rs3_cmdreg);
		}
	}
	...
	if (send_len != 0 && (sts & STS_TXRDY)) {
		outp2(IOP_RS3_DAT, *send_ptr++);		/* 次の１文字送信 */
		if (--send_len == 0) {
			if (duplex == DUPLEX_HALF) {
				/* 送信禁止 */
				outp2(IOP_RS3_MOD, rs3_cmdreg &= ~CMD_TXE);
			...
			outp2(IOP_RS3_IMR, imreg = imreg & ~IMR_TXRE | IMR_TXEE);
			free(send_buf);		/* 送信バッファ解放 */
		}
	}

	/* 割り込みマスク */
	outp2(IOP_RS3_IMR, 0x00);
