 ...
 * 業種別アクセス推移表を作成
 */
public class CategoryAccessBean implements I_Access_Page {
	...
	public void searchDB(DataBase db, SelectCondBean selectcondBean) {
		try {
			...
			where.append("and [PF]_CORNER_CODE='" + cornerCode + "'\n");
			if (!category.equals("/"))
        ...
        sql.append(CommonSql.setAttributeSql2(selectcondBean));
			...
			q01.append(where);
			q01.append("group by [PF]_ACCESS[TB]_DATE," + s_code + " \n");
			...
			while (rs.next()) {
				...
				tags.append(CommonFunc.getCountValue(counterTbl, dates, categoryCodes, true, rankFlag));//アクセス数の多い１-3位に色をつける
