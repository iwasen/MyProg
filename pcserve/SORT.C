...
static	SHORT	merge(int, int, SORT_BLOCK *, SORT_BLOCK *, CHAR *, int, int (*)(CHAR *, CHAR *));
static	SHORT	wrt_index(IDX *, int, LONG, int);
static	SHORT	wrt_block(IDX *, int, int, int, CHAR *, LONG *, LONG *);
...
/*=======================================================================
 ...
 |
 =======================================================================*/
static	SHORT	merge(int fh1, int fh2, SORT_BLOCK *sbp1, SORT_BLOCK *sbp2,
			CHAR *bufp, int lRec, int (*comp)(CHAR *, CHAR *))
{
	...
	offset2 = sbp2->offset;
	buf1 = bufp;
	buf2 = buf1 + buf_size / 3;
	buf3 = buf2 + buf_size / 3;
...
SHORT	IDXMake(PDBF dp, PIDX ip)
{
	...
	if ((fh[0] = __CREAT_LOCAL(tmpFile[0])) == -1) {
	...
	if ((fh[1] = __CREAT_LOCAL(tmpFile[1])) == -1) {
		err = ERROR_DB_TMPFILE;
		goto ret;
	}
	...
	for (block = 0; block < nBlock; block++) {
		...
		qsort(bufp, n, lRec, (int (_cdecl *)(const void *,const void *))comp);
	...
	for ( ; nBlock != 1; nBlock = (nBlock + 1) / 2) {
		...
		for (block = 0; block < nBlock/2; block++) {
			...
			if ((err = merge(fh[tmp1], fh[tmp2], &sbp[block1],
					&sbp[block2], bufp, lRec, (int (_cdecl *)(char *,char *))comp)) != 0)
				goto ret;
...
/*=======================================================================
 ...
 |
 =======================================================================*/
static	SHORT	wrt_index(PIDX ip, int fh, LONG nRec, int lRec)
{
	PIDX_H	ihp;
	LONG	bno;
	LONG	rno;
...
/*=======================================================================
 ...
 |
 =======================================================================*/
static	SHORT	wrt_block(PIDX ip, int fh, int level, int lRec, CHAR *key
		, LONG *bno, LONG *rno)
{
	PIDX_H	ihp;
	PIDX_B	bp;
