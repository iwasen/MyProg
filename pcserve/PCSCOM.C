...
void GetFullPath(char *pPathNameSrc, char *pPathNameDst, int size)
{
	char pathName[256];
	int len;
	char *p;

	if ((p = strchr(pPathNameSrc, ',')) != NULL) {
		len = p - pPathNameSrc;
		memcpy(pathName, pPathNameSrc, len);
		pathName[len] = '\0';
	} else
		strcpy(pathName, pPathNameSrc);

	_fullpath(pPathNameDst, pathName, size);
	StringToUpper(pPathNameDst);
}

LONG lseek64(HANDLE hFile, LONG64 qwOffset, DWORD dwMoveMethod)
{
	LARGE_INTEGER int64;

	int64.QuadPart = qwOffset;
	return SetFilePointer(hFile, int64.LowPart, &int64.HighPart, dwMoveMethod);
}
