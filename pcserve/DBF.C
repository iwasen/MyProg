...
static	SHORT	_DBFOpen(CHAR *fileName, PDBF *dpp)
{
	...
	for (dp = pTopDBF; dp != NULL; dp = dp->chain) {
		if (strcmp(pathName, dp->fileName) == 0) {
	...
	strcpy(dp->fileName, pathName);	/* ファイル名セーブ */
	...
	/* レコード数調整 */
	nRec = (__LSEEK(dp->fh, 0L, SEEK_END) - dhp->lHeader) / dhp->lRec;
	if (nRec < 0)
		nRec = 0;
	if (nRec > dhp->nRec) {
...
SHORT	DBFWrite(PDBF dp, LONG rno, CHAR *bufp)
{
	...
	/* 書き込み */
	rc = __WRITE2(dp->fileName, dp->fh, bufp, dhp->lRec,
...
SHORT	DBFWrite2(PDBF dp, LONG rno, CHAR *bufp)
{
	...
	/* 書き込み */
	rc = __WRITE2(dp->fileName, dp->fh, bufp, dp->lRec,
...
/*=======================================================================
 ...
 |
 |		PDBF	dp;		ＤＢＦポインタ
 |		BOOL	bReopen;	再オープンフラグ
 |
 |		返値			エラーコード
 |
 =======================================================================*/
SHORT	DBFFlush(PDBF dp, BOOL bReopen)
{
...
SHORT	DBFPack(PDBF dp)
{
	...
	if (bfp != NULL && bfp->bhp->freeBlock != 0) {
		...
		// DBF バックアップコピー
		__CLOSE(dp->fh);
		bRc = CopyFile(dp->fileName, dbfBackupFileName, FALSE);
		dp->fh = __OPEN(dp->fileName);
		if (!bRc) {
			free(rbp);
...
end_binpack:
		if (err == 0) {
			...
			MoveFile(binTempFileName, binFileName);
			BINOpen(binFileName, &dp->bfp);
			DeleteFile(dbfBackupFileName);
		} else {
			BINClose(bfpTemp);
			DeleteFile(binTempFileName);

			__CLOSE(dp->fh);
			DeleteFile(dp->fileName);
			MoveFile(dbfBackupFileName, dp->fileName);
			dp->fh = __OPEN(dp->fileName);
		}
	}	
