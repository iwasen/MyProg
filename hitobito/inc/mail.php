...
// ■メンバー宛メール
function send_mail_member($send_mail_id, $to, $sub_seq = '', $member_id = '') {
	if ($member_id)
		$mail_header = "X-Member-Id: $member_id";

	return send_mail_list($send_mail_id, $to, $sub_seq, $mail_header);
}
...
// ■１メール送信
function send_mail_one($subject, $from, $to, $reply_to, $body, $cc='', $bcc='', $mail_header='') {
	if ($cc != '')
		$mail_header .= "Cc: $cc\n";
	if ($bcc != '')
		$mail_header .= "Bcc: $bcc\n";
...
function get_mail_template($template_id, &$subject, &$from, &$cc, &$bcc, &$repty_to, &$body) {
	$sql = "SELECT * FROM m_mail_template WHERE mtm_template_id='$template_id'";
	$result = db_exec($sql);
	if (pg_num_rows($result)) {
		$fetch = pg_fetch_object($result, 0);
		$subject = $fetch->mtm_subject;
		$from = $fetch->mtm_from;
		$cc = $fetch->mtm_cc;
		$bcc = $fetch->mtm_bcc;
		$repty_to = $fetch->mtm_reply_to;
		$body = str_replace("\r\n", "\n", $fetch->mtm_body);
	}
}
...
// ■Message-ID:生成
function make_message_id($str = 'hitobito') {
	return uniqid($str, true) . '@' . get_const('room_mail_domain');
}
...
// ■メールヘッダ生成
function make_mail_header($message_id, $ref_message_id, $room_id = '') {
