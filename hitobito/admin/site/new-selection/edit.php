...
// ページヘッダ情報設定
$header = new header_class('ナビセレクト最新投稿', BACK_TOP);
...
$max_navi_select = get_const('max_navi_select');

// 上位20件取得サブクエリ
$sb1 = "SELECT bla_article_id AS sb1_article_id"
		. " FROM t_blog_article"
		. ($channel_id ? " JOIN t_navi_page ON nvp_blog_id=bla_blog_id" : "")
		. " WHERE bla_status=1 AND bla_open_flag=1 AND bla_navi_select_flag=1" . ($channel_id ? " AND nvp_channel_id=$channel_id" : "")
...
$sql = "SELECT bla_article_id, chn_channel_name, nvp_title, bla_title, bla_date, pvc_pv_count, ($sb2) AS trackback_count, ($sb3) AS comment_count, bla_blog_id"
		. " FROM ($sb1) sb1"
...
for ($i = 0; $i < $nrow; $i++) {
	...
	$list->set_data(--$max_navi_select >= 0 ? '掲載中' : '');
	$list->set_html(html_input_checkbox('navi_select_flag[]', $fetch->bla_article_id, false, 'height:13px'));
}
?>
<html lang="ja">
<head>
...
function onsubmit_edit(f) {
	f.action = "update.php";
	return confirm("ナビセレクト最新投稿を更新します。よろしいですか？");
}
