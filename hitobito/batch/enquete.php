...
function start_enquete($date) {
	...
	for ($i = 0; $i < $nrow; $i++) {
		...
		if ($err_msg == '') {
			...
			$batch->set_log(0, "ナビアンケート（ID={$enquete_id}）を開始しました");
		} else {
			db_rollback();

			$batch->set_log(9, "ナビアンケート開始処理でDBエラーが発生しました。($err_msg)");
		}
	}
}
...
function end_enquete($date) {
	...
	for ($i = 0; $i < $nrow; $i++) {
		...
		if ($err_msg == '') {
			...
			$batch->set_log(0, "ナビアンケート（ID={$enquete_id}）を終了しました");
		} else {
			db_rollback();

			$batch->set_log(9, "ナビアンケート終了処理でDBエラーが発生しました。($err_msg)");
		}
	}
}
?>