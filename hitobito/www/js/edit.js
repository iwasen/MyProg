...
function alignleftThis() { insertAroundCaret('<div align="left">','</div>'); }
function alignrightThis() { insertAroundCaret('<div align="right">','</div>'); }
function aligncenterThis() { insertAroundCaret('<div align="center">','</div>'); }
function colorThis(color) { insertAroundCaret('<font color="'+color+'">','</font>'); }
function imageThis() {
  ele = window.document.storyform.image_selecter;
    if(ele.options[ele.selectedIndex].value ==0) return;
  insertAtCaret('<img src="blogimage.php?id='+ ele.options[ele.selectedIndex].value+'" />');
}
...
function execAndUpdate(action) {
	laxxxaretPos.execCommand(action);
	updAllPreviews();	
}
...
// stores the caret
var lastSelected, laxxxaretPos;
function storeCaret (textEl) {

	// store caret
	if (textEl.createTextRange) 
		//textEl.caretPos = document.selection.createRange().duplicate();
		laxxxaretPos = document.selection.createRange().duplicate();
...
function insertAtCaret (text) {
	...
	if (textEl && textEl.createTextRange && laxxxaretPos) {
		var caretPos = laxxxaretPos;
...
function getCaretText() {
	if (!document.all && document.getElementById)
		...
		return laxxxaretPos.text;
}

function isCaretEmpty() {
	if (lastSelected && lastSelected.createTextRange && laxxxaretPos)
		return (laxxxaretPos.text == '');
	else if (!document.all && document.getElementById)
		return (mozSelectedText() == '');
	else
