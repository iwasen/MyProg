...
function& adodb_log_sql(&$conn,$sql,$inputarr)
{
	...
	if (!empty($conn->_logsql)) {
		...
		if ($ok) {
			...
			if (!$ok) {
				ADOConxxxtion::outp( "<p><b>LOGSQL Insert Failed</b>: $isql<br>$err2</p>");
				$conn->_logsql = false;
			}
		}
		$conn->_errorMsg = $errM;
...
<table border=1 bgcolor=white><tr><td><b>Load</b><td><b>Count</b><td><b>SQL</b><td><b>Max</b><td><b>Min</b></tr>\n";
	...
	*/
	function &PollParameters()
	{
		...
		$arr[3] = (integer) $this->DBParameter('current conxxxtions');
		return $arr;
	}
...
<?php
   ...
   /************************************************************************/
    ...
    {
        ADOConxxxtion::outp( sprintf( "<p>%s: '%s' not implemented for driver '%s'</p>", __CLASS__, __FUNCTION__, $this->conn->databaseType));
        return false;
    }
