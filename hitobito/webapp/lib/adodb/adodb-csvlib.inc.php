...
*/
	...
	function _rs2serialize(&$rs,$conn=false,$sql='')
	{
		...
		$class = $rs->conxxxtion->arrayClass;
		$rs2 = new $class();
		$rs2->sql = $rs->sql;
	...
	*/
	...
	# 4. another process creates $filename between unlink() and rename() -- ok, rename() fails and  cache updated
		if (strncmp(PHP_OS,'WIN',3) === 0) {
			...
			if (!$ok) {
				if ($debug) ADOConxxxtion::outp( " Rename $tmpname ".($ok? 'ok' : 'failed'));
			}
			return $ok;
		}
		...
		if (flock($fd, LOCK_EX) && ftruncate($fd, 0)) {
			...
			fclose($fd);
			if ($debug)ADOConxxxtion::outp( " Failed acquiring lock for $filename<br>\n");
			$ok = false;
		}
