...
FROM `nuke_stories` `t1`, `nuke_authors` `t2`, `nuke_stories_cat` `t3`, `nuke_topics` `t4` 
	...
	flush();
	// mssql too slow in failing bad conxxxtion
	if (false && $db->databaseType != 'mssql') {
		print "<p>Testing bad conxxxtion. Ignore following error msgs:<br>";
		$db2 = ADONewConxxxtion();
		$rez = $db2->Conxxxt("bad conxxxtion");
		$err = $db2->ErrorMsg();
		print "<i>Error='$err'</i></p>";
		if ($rez) print "<b>Cannot check if conxxxtion failed.</b> The Conxxxt() function returned true.</p>";
	}
	error_reporting($e);
	flush();
...
*/
 ...
 //////////////////////////////////////////////////////////////////////////////////////////
	...
	if ($rs) {
		if (empty($rs->conxxxtion)) print "<b>Conxxxtion object missing from recordset</b></br>";
 ...
 //	$db->debug=true;
	...
	print "<p>Test SQLDate: ".htmlspecialchars($sql)."</p>";
	$rs = $db->SelectLimit($sql,1);
	$ts = ADOConxxxtion::UnixDate('1974-02-25');
	...
	print "<p>Testing Bad Conxxxtion</p>";
	flush();
	
	if (true || PHP_VERSION < 5)  {
		if ($db->dataProvider == 'odbtp') $db->databaseType = 'odbtp';
		$conn = NewADOConxxxtion($db->databaseType);
		$conn->raiseErrorFn = 'adodb_test_err';
		if (1) $conn->PConxxxt('abc','baduser','badpassword');
...
echo "<br>vers=",ADOConxxxtion::Version();
