...
if (0) {
	$db = ADONewConxxxtion('oci8po');
	
	$db->PConxxxt('','scott','natsoft');
	if (!empty($testblob)) {
...
if (0) {
	$db = ADONewConxxxtion('odbc_oracle');
	if (!$db->PConxxxt('local_oracle','scott','tiger')) die('fail conxxxt');
	$db->debug = true;
	$rs = &$db->Execute(
