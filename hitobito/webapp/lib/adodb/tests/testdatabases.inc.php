...
if (!empty($testpostgres)) {
	...
	print "<h1>Conxxxting $db->databaseType...</h1>";
	if ($db->Conxxxt("localhost","tester","test","test")) {
...
if (!empty($testpgodbc)) { 
	...
	$db = &ADONewConxxxtion('odbc');
	$db->hasTransactions = false;
	print "<h1>Conxxxting $db->databaseType...</h1>";
	
	if ($db->PConxxxt('Postgresql')) {
...
if (!empty($testibase)) {
	//$_GET['nolog'] = true;
	$db = &ADONewConxxxtion('firebird');
	print "<h1>Conxxxting $db->databaseType...</h1>";
	if ($db->PConxxxt("localhost:d:\\firebird\\151\\examples\\EMPLOYEE.fdb", "sysdba", "masterkey", ""))
...
if (!empty($testsqlite)) {
	...
	$dsn = "sqlite://$path/";
	$db = ADONewConxxxtion($dsn);
	echo $dsn;
	
//	$db = &ADONewConxxxtion('sqlite');
	print "<h1>Conxxxting $db->databaseType...</h1>";
	
	if (1)
	//if ($db->PConxxxt("d:\\inetpub\\adodb\\sqlite.db", "", "", ""))
...
if (!empty($testpdopgsql)) {
	...
	$u = 'tester';$p='test';
	$db = &ADONewConxxxtion('pdo');
	print "<h1>Conxxxting $db->databaseType...</h1>";
	$db->Conxxxt($connstr,$u,$p) || die("CONxxxT FAILED");
	testdb($db,
...
if (!empty($testaccess) && !empty($testado)) { // ADO ACCESS
	...
	$db = &ADONewConxxxtion("ado_access");
	print "<h1>Conxxxting $db->databaseType...</h1>";
	...
	$myDSN =  'PROVIDER=Microsoft.Jet.OLEDB.4.0;'
		...
		//. 'USER ID=;PASSWORD=;';
	$_GET['nolog'] = 1;
	if ($db->PConxxxt($myDSN, "", "", "")) {
		print "ADO version=".$db->_conxxxtionID->version."<br>";
		testdb($db,"create table ADOXYZ (id int, firstname char(24), lastname char(24),created datetime)");
...
if (!empty($testvfp)) { // ODBC
	$db = &ADONewConxxxtion('vfp');
	print "<h1>Conxxxting $db->databaseType...</h1>";flush();

	if ( $db->PConxxxt("vfp-adoxyz")) {
	...
	echo "<hr>";
	$db = &ADONewConxxxtion('odbtp');
	
	if ( $db->PConxxxt('localhost','DRIVER={Microsoft Visual FoxPro Driver};SOURCETYPE=DBF;SOURCEDB=d:\inetpub\adodb;EXCLUSIVE=NO;')) {
	print "<h1>Conxxxting $db->databaseType...</h1>";flush();
	testdb($db,"create table d:\\inetpub\\adodb\\ADOXYZ (id int, firstname char(24), lastname char(24),created date)");
...
if (!empty($testmysql)) { // MYSQL
	...
	$db = &ADONewConxxxtion("mysqlt://$user:$password@$server/$database?persist");
	print "<h1>Conxxxting $db->databaseType...</h1>";
	
	if (true || $db->PConxxxt($server, "root", "", "northwind")) {
...
if (!empty($testmysqli)) { // MYSQL
	...
	$db = &ADONewConxxxtion('mysqli');
	print "<h1>Conxxxting $db->databaseType...</h1>";
	if (PHP_VERSION >= 5 || $_SERVER['HTTP_HOST'] == 'localhost') $server = 'localhost';
	else $server = "mangrove";
	if ($db->PConxxxt($server, "root", "", "northwind")) {
		//$db->debug=1;$db->Execute('drop table ADOXYZ');
		testdb($db,
		"create table ADOXYZ (id int, firstname char(24), lastname char(24), created date)");
	} else print "ERROR: MySQL test requires a MySQL server on localhost, userid='admin', password='', database='test'".'<BR>'.$db->ErrorMsg();
...
if (!empty($testproxy)){
	$db = &ADONewConxxxtion('proxy');
	print "<h1>Conxxxting $db->databaseType...</h1>";
	if ($_SERVER['HTTP_HOST'] == 'localhost') $server = 'localhost';

	if ($db->PConxxxt('http://localhost/php/phplens/adodb/server.php'))
		testdb($db,
		"create table ADOXYZ (id int, firstname char(24), lastname char(24), created date) type=innodb");
	else print "ERROR: MySQL test requires a MySQL server on localhost, userid='admin', password='', database='test'".'<BR>'.$db->ErrorMsg();
...
if (!empty($testoracle)) {
	...
	$db = ADONewConxxxtion($dsn);
	print "<h1>Conxxxting $db->databaseType...</h1>";
	if (true || $db->Conxxxt('', "scott", "natsoft",''))
...
if (false && !empty($testoracle)) { 
	...
	$db = ADONewConxxxtion();
	print "<h1>Conxxxting $db->databaseType...</h1>";
	if ($db->PConxxxt("", "scott", "tiger", "natsoft.domain"))
		testdb($db,"create table ADOXYZ (id int, firstname varchar(24), lastname varchar(24),created date)");
...
if (!empty($testmssql)) { // MS SQL Server via ODBC
	...
	if ($db->PConxxxt($dsn, "adodb", "natsoft", ""))  {
...
if (!empty($testmssql) && !empty($testado) ) { // ADO ACCESS MSSQL -- thru ODBC -- DSN-less
	...
	$db = &ADONewConxxxtion("ado_mssql");
	//$db->debug=1;
	print "<h1>Conxxxting DSN-less $db->databaseType...</h1>";
	
	$myDSN="PROVIDER=MSDASQL;DRIVER={SQL Server};"
		. "SERVER=$server;DATABASE=NorthWind;UID=adodb;PWD=natsoft;Trusted_Conxxxtion=No";
...
if (!empty($testmssql)) { // MS SQL Server -- the extension is buggy -- probably better to use ODBC
	$db = ADONewConxxxtion("mssqlpo");
	//$db->debug=1;
	print "<h1>Conxxxting $db->databaseType...</h1>";
	
	$ok = $db->Conxxxt('','adodb','natsoft','northwind');

	if ($ok or $db->PConxxxt("mangrove", "sa", "natsoft", "ai")) {
...
if (!empty($testmssql) && !empty($testado)) { // ADO ACCESS MSSQL with OLEDB provider
	...
	$db = &ADONewConxxxtion("ado_mssql");
	print "<h1>Conxxxting DSN-less OLEDB Provider $db->databaseType...</h1>";
	//$db->debug=1;
	$myDSN="SERVER=localhost;DATABASE=northwind;Trusted_Conxxxtion=yes";
	if ($db->PConxxxt($myDSN, "adodb", "natsoft", 'SQLOLEDB')) {
		testdb($db,"create table ADOXYZ (id int, firstname char(24), lastname char(24),created datetime)");
