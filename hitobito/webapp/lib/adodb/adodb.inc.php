...
/**
	...
	ADO, SAP DB, SQLite and ODBC. We have had successful reports of conxxxting to Progress and
 ...
 if (!defined('_ADODB_LAYER')) {
	...
	*/
	...
	if (!$ADODB_EXTENSION || ADODB_EXTENSION < 4.0) {
		...
		define('ADODB_BAD_RS','<p>Bad $rs in %s. Conxxxtion or SQL invalid. Try using $conxxxtion->debug=true;</p>');
...
/*
		...
		// so we can as well make not_null standard (leaving it at "false" does not hxxx anyways)
...
*/
	...
	function ADODB_TransMonitor($dbms, $fn, $errno, $errmsg, $p1, $p2, &$thisConxxxtion)
	{
		//print "Errorno ($fn errno=$errno m=$errmsg) ";
		$thisConxxxtion->_transOK = false;
		if ($thisConxxxtion->_oldRaiseFn) {
			$fn = $thisConxxxtion->_oldRaiseFn;
			$fn($dbms, $fn, $errno, $errmsg, $p1, $p2,$thisConxxxtion);
		}
	}
	...
	/**
	 * Conxxxtion object. For conxxxting to databases, and executing queries.
	 */ 
	class ADOConxxxtion {
	//
	// PUBLIC VARS 
	...
	var $user = ''; 			/// The username which is used to conxxxt to the database server. 
	...
	var $autoRollback = false; // autoRollback on PConxxxt().
	...
	var $_transOK = null;
	var $_conxxxtionID	= false;	/// The returned link identifier whenever a successful database conxxxtion is made.	
	...
	var $_isPersistentConxxxtion = false;	/// A boolean variable to state whether its a persistent conxxxtion or normal conxxxtion.	*/
	var $_bindInputArray = false; /// set to true if ADOConxxxtion.Execute() permits binding of array parameters.
	var $_evalAll = false;
	var $_affected = false;
	var $_logsql = false;
	...
	*/
	...
	function IsConxxxted()
	{
    	return !empty($this->_conxxxtionID);
	}
	...
	/**
	* All error messages go through this bottlexxxk function.
	...
	*/
	 ...
	 * Conxxxt to database
	 *
	 * @param [argHostname]		Host to conxxxt to
	 * @param [argUsername]		Userid to login
	 * @param [argPassword]		Associated password
	 * @param [argDatabaseName]	database
	 * @param [forceNew]		force new conxxxtion
	 *
	 * @return true or false
	 */	  
	function Conxxxt($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "", $forceNew = false) 
	{
		...
		$this->_isPersistentConxxxtion = false;	
		if ($forceNew) {
			if ($rez=$this->_nconxxxt($this->host, $this->user, $this->password, $this->database)) return true;
		} else {
			 if ($rez=$this->_conxxxt($this->host, $this->user, $this->password, $this->database)) return true;
		}
		if (isset($rez)) {
			$err = $this->ErrorMsg();
			if (empty($err)) $err = "Conxxxtion error to server '$argHostname' with user '$argUsername'";
			$ret = false;
		} else {
		...
		if ($fn = $this->raiseErrorFn) 
			$fn($this->databaseType,'CONxxxT',$this->ErrorNo(),$err,$this->host,$this->database,$this);
		
		
		$this->_conxxxtionID = false;
		if ($this->debug) ADOConxxxtion::outp( $this->host.': '.$err);
		return $ret;
	}	
	
	function _nconxxxt($argHostname, $argUsername, $argPassword, $argDatabaseName)
	{
		return $this->_conxxxt($argHostname, $argUsername, $argPassword, $argDatabaseName);
	}
	
	
	/**
	 * Always force a new conxxxtion to database - currently only works with oracle
	 *
	 * @param [argHostname]		Host to conxxxt to
	 * @param [argUsername]		Userid to login
	 * @param [argPassword]		Associated password
	 ...
	 * @return true or false
	 */	  
	function NConxxxt($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "") 
	{
		return $this->Conxxxt($argHostname, $argUsername, $argPassword, $argDatabaseName, true);
	}
	
	/**
	 * Establish persistent conxxxt to database
	 *
	 * @param [argHostname]		Host to conxxxt to
	 * @param [argUsername]		Userid to login
	 * @param [argPassword]		Associated password
	 ...
	 */	
	function PConxxxt($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "")
	{
		if (defined('ADODB_NEVER_PERSIST')) 
			return $this->Conxxxt($argHostname,$argUsername,$argPassword,$argDatabaseName);
		...
		$this->_isPersistentConxxxtion = true;	
		if ($rez = $this->_pconxxxt($this->host, $this->user, $this->password, $this->database)) return true;
		if (isset($rez)) {
			$err = $this->ErrorMsg();
			if (empty($err)) $err = "Conxxxtion error to server '$argHostname' with user '$argUsername'";
			$ret = false;
		...
		if ($fn = $this->raiseErrorFn) {
			$fn($this->databaseType,'PCONxxxT',$this->ErrorNo(),$err,$this->host,$this->database,$this);
		}
	...
	*/
	...
	/**
	 Requested by "Karsten Dambekalns" <k.dambekalns@fishfxxx.de>
	*/
	function QMagic($s)
	{
	...
	*/
	function StartTrans($errfn = 'ADODB_TransMonitor')
	{
		...
		if ($this->debug && $this->transCnt > 0) ADOConxxxtion::outp("Bad Transaction: StartTrans called within BeginTrans");
		$this->BeginTrans();
		$this->transOff = 1;
	}
	...
	/**
		Used together with StartTrans() to end a transaction. Monitors conxxxtion
	...
	*/
	function CompleteTrans($autoComplete = true)
	{
		...
		if ($this->_transOK && $autoComplete) {
			if (!$this->CommitTrans()) {
				$this->_transOK = false;
				if ($this->debug) ADOConxxxtion::outp("Smart Commit failed");
			} else
				if ($this->debug) ADOConxxxtion::outp("Smart Commit occurred");
		} else {
			$this->_transOK = false;
			$this->RollbackTrans();
			if ($this->debug) ADOCOnxxxtion::outp("Smart Rollback occurred");
		}
	...
	*/
	function FailTrans()
	{
		if ($this->debug) 
			if ($this->transOff == 0) {
				ADOConxxxtion::outp("FailTrans outside StartTrans/CompleteTrans");
			} else {
				ADOConxxxtion::outp("FailTrans was called");
				adodb_backtrace();
			}
		$this->_transOK = false;
	}
	...
	*/
		...
		if ($inputarr) {
			...
			if (!is_array($sql) && !$this->_bindInputArray) {
				...
				foreach($inputarr as $arr) {
					...
					if (isset($sqlarr[$i])) {
						$sql .= $sqlarr[$i];
						if ($i+1 != sizeof($sqlarr)) ADOConxxxtion::outp( "Input Array does not match ?: ".htmlspecialchars($sql));
					} else if ($i != sizeof($sqlarr))	
						ADOConxxxtion::outp( "Input array does not match ?: ".htmlspecialchars($sql));
	...
	function &_Execute($sql,$inputarr=false)
	{
		...
		*************************/
		...
		$rs->conxxxtion = &$this; // Pablo suggestion
		$rs->Init();
	...
	function Insert_ID($table='',$column='')
	{
		...
		if ($this->debug) {
			ADOConxxxtion::outp( '<p>Insert_ID error</p>');
			adodb_backtrace();
		}
		return false;
	}
	...
	*/ 
	function Affected_Rows()
	{
		...
		if ($this->debug) ADOConxxxtion::outp( '<p>Affected_Rows error</p>',false);
		return false;
	}
	...
	/**
	 * Choose a database to conxxxt to. Many databases do not support this.
	 *
	...
	* @return		the recordset ($rs->databaseType == 'array')
	...
	/**
	* Create serializable recordset. Breaks rs link to conxxxtion.
	*
	...
	* @return		the recordset ($rs->databaseType == 'array')
	...
	{	
		if (!is_numeric($secs2cache)) {
			...
			if ($sql === false) ADOConxxxtion::outp( "Warning: \$sql missing from CacheSelectLimit()");
	...
	* If $sql == false, then we purge all files in the cache.
	...
	global $ADODB_CACHE_DIR;
		...
		if (strlen($ADODB_CACHE_DIR) > 1 && !$sql) {
			...
			if ($this->debug) {
				ADOConxxxtion::outp( "CacheFlush: $cmd<br><pre>\n", system($cmd),"</pre>");
			} else {
				exec($cmd);
			}
			return;
		...
		if (!@unlink($f)) {
			if ($this->debug) ADOConxxxtion::outp( "CacheFlush: failed for $f");
		}
	}
	...
	* then we can scale to 12.8 million unique cached recordsets. Wow!
	...
	static $notSafeMode;
		...
		if ($createdir && $notSafeMode && !file_exists($dir)) {
			$oldu = umask(0);
			if (!mkdir($dir,0771)) 
				if ($this->debug) ADOConxxxtion::outp( "Unable to mkdir $dir for $sql");
			umask($oldu);
		}
	...
	function &CacheExecute($secs2cache,$sql=false,$inputarr=false)
	{
		...
		// no cached rs found
			if ($this->debug) {
				if (get_magic_quotes_runtime()) {
					ADOConxxxtion::outp("Please disable magic_quotes_runtime - it corrupts cache files :(");
				}
				if ($this->debug !== -1) ADOConxxxtion::outp( " $md5file cache failure: $err (see sql below)");
			}
			...
			if ($rs) {
				...
				if (!adodb_write_file($md5file,$txt,$this->debug)) {
					...
					if ($this->debug) ADOConxxxtion::outp( " Cache write error");
				}
				if ($rs->EOF && !$eof) {
					$rs->MoveFirst();
		...
		// ok, set cached object found
			...
			if ($this->debug){ 
				...
				ADOConxxxtion::outp( " $md5file reloaded, ttl=$ttl [ $s ]");
			}
		}
		return $rs;
	}
	...
	function& AutoExecute($table, $fields_values, $mode = 'INSERT', $where = FALSE, $forceUpdate=true, $magicq=false) 
	{
		...
		if ($where!==FALSE) $sql .= ' WHERE '.$where;
			...
			ADOConxxxtion::outp('AutoExecute: Illegal mode=UPDATE with empty WHERE clause');
			return false;
		}
		...
		case '1':
			...
			break;
		default:
			ADOConxxxtion::outp("AutoExecute: Unknown mode=$mode");
			return false;
		}
		$ret = false;
	...
	*/
	...
	/**
	*  Change the SQL conxxxtion locale to a specified locale.
	...
	*/
	...
	/**
	 * return the databases that the driver can conxxxt to. 
	...
	function DBTimeStamp($ts)
	{
		...
		$ts = ADOConxxxtion::UnixTimeStamp($ts);
		return adodb_date($this->fmtTimeStamp,$ts);
	}
   ...
   /**
	...
	var $conxxxtion = false; /// the parent conxxxtion
	/*
	 *	private variables	
	 */
	 ...
	 * Constructor
	 *
	 * @param queryID  	this is the queryID returned by ADOConxxxtion->_query()
	 *
	 */
	...
	*/
	...
	/**
	 * return recordset as a 2-dimensional array. 
	 * Helper function for ADOConxxxtion->SelectLimit()
	 *
	 * @param offset	is the row to start calculations from (1-based)
	...
	function UnixDate($v)
	{
		return ADOConxxxtion::UnixDate($v);
	}
	...
	function UnixTimeStamp($v)
	{
		return ADOConxxxtion::UnixTimeStamp($v);
	}
   ...
   */
	...
	function Close() 
	{
		// free conxxxtion object - this seems to globally free the object
		// and not merely the reference, so don't do this...
		// $this->conxxxtion = false; 
		if (!$this->_closed) {
			$this->_closed = true;
			return $this->_close();		
		} else
	...
	*/
	...
	function PO_RecordCount($table="", $condition="") {
		...
		if ($lnumrows == -1 && $this->conxxxtion) {
			IF ($table) {
				if ($condition) $condition = " WHERE " . $condition; 
				$resultrows = &$this->conxxxtion->Execute("SELECT COUNT(*) FROM $table $condition");
	...
	*/
	...
	global $ADODB_LASTDB;
		...
		//ADOConxxxtion::outp(adodb_pr(get_declared_classes(),true));
		if (!file_exists($file)) ADOConxxxtion::outp("Missing file: $file");
		else ADOConxxxtion::outp("Syntax error in file: $file");
		return false;
	}

	/**
	 * synonym for ADONewConxxxtion for people like me who cannot remember the correct name
	 */
	function &NewADOConxxxtion($db='')
	{
		$tmp =& ADONewConxxxtion($db);
		return $tmp;
	}
	
	/**
	 * Instantiate a new Conxxxtion class for a specific database driver.
	 *
	 * @param [db]  is the database Conxxxtion object to create. If undefined,
	 * 	use the last database driver that was loaded by ADOLoadCode().
	 *
	 * @return the freshly created instance of the Conxxxtion class.
	 */
	function &ADONewConxxxtion($db='')
	{
	GLOBAL $ADODB_NEWCONxxxTION, $ADODB_LASTDB;
	 ...
	 */
		if (!empty($ADODB_NEWCONxxxTION)) {
			$obj = $ADODB_NEWCONxxxTION($db);
			...
			if (!$db) {
				...
				if ($errorfn) {
					// raise an error
					$ignore = false;
					$errorfn('ADONewConxxxtion', 'ADONewConxxxtion', -998,
							 "could not load the database driver for '$db'",
							 $db,false,$ignore);
				} else
					 ADOConxxxtion::outp( "<p>ADONewConxxxtion: Unable to load database driver '$db'</p>",false);
		...
		if ($obj) {
			...
			if (isset($dsna)) {
				...
				if (empty($persist))
					$ok = $obj->Conxxxt($dsna['host'], $dsna['user'], $dsna['pass'], $dsna['path']);
				else
					$ok = $obj->PConxxxt($dsna['host'], $dsna['user'], $dsna['pass'], $dsna['path']);
	...
	function &NewDataDictionary(&$conn,$drivername=false)
	{
		...
		if (!file_exists($path)) {
			ADOConxxxtion::outp("Database driver '$path' not available");
			return $false;
		}
		include_once($path);
		...
		$dict->conxxxtion = &$conn;
		$dict->upperName = strtoupper($drivername);
		$dict->quote = $conn->nameQuote;
		if (!empty($conn->_conxxxtionID))
