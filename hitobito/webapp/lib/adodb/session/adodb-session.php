...
class ADODB_Session {
	...
	*/
	function host($host = null) {
		...
		if (!is_null($host)) {
			...
			// backwards compatibility
			if (isset($GLOBALS['ADODB_SESSION_CONxxxT'])) {
				return $GLOBALS['ADODB_SESSION_CONxxxT'];
			}
		}
	...
	*/
	...
	/*!
		...
		If $conn already exists, reuse that conxxxtion
	*/
...
#		assert('$host');
		...
		$conn =& ADONewConxxxtion($driver);

		if ($debug) {
			$conn->debug = true;
//			ADOConxxxtion::outp( " driver=$driver user=$user pwd=$password db=$database ");
		}

		if ($persist) {
			switch($persist) {
			default:
			case 'P': $ok = $conn->PConxxxt($host, $user, $password, $database); break;
			case 'C': $ok = $conn->Conxxxt($host, $user, $password, $database); break;
			case 'N': $ok = $conn->NConxxxt($host, $user, $password, $database); break;
			}
		} else {
			$ok = $conn->Conxxxt($host, $user, $password, $database);
		}

		if ($ok) $GLOBALS['ADODB_SESS_CONN'] =& $conn;
		else
			ADOConxxxtion::outp('<p>Session: conxxxtion failed</p>', false);
...
*/
	...
	*/
	function write($key, $val) {
		...
		if (!$rs) {
			ADOConxxxtion::outp('<p>Session Replace: ' . $conn->ErrorMsg() . '</p>', false);
			return false;
		}  else {
	...
	*/
	function gc($maxlifetime) {
		...
		if ($expire_notify) {
			...
			if ($debug) {
				ADOConxxxtion::outp("<p><b>Garbage Collection</b>: $sql</p>");
			}
		}
