...
To force non-persistent conxxxtions, call adodb_session_open first before session_start():
...
-- \$_SESSION['AVAR']={$_SESSION['AVAR']}</p>";
 ...
 1. Create this table in your database (syntax might vary depending on your db):
  ...
  2. Then define the following parameters. You can either modify
	...
	$ADODB_SESSION_CONxxxT='server to conxxxt to';
...
GLOBAL 	$ADODB_SESSION_CONxxxT, 
	...
	if (empty($ADODB_SESSION_DRIVER)) {
		...
		$ADODB_SESSION_CONxxxT='localhost';
	...
	$ADODB_SESS['conxxxt'] = $ADODB_SESSION_CONxxxT;
...
/****************************************************************************************\
	Create the conxxxtion to the database. 
	
	If $ADODB_SESS_CONN already exists, reuse that conxxxtion
\****************************************************************************************/
...
GLOBAL 	$ADODB_SESSION_CONxxxT, 
	...
	$ADODB_SESS_CONN = ADONewConxxxtion($ADODB_SESSION_DRIVER);
	if (!empty($ADODB_SESS_DEBUG)) {
		$ADODB_SESS_CONN->debug = true;
		ADOConxxxtion::outp( " conn=$ADODB_SESSION_CONxxxT user=$ADODB_SESSION_USER pwd=$ADODB_SESSION_PWD db=$ADODB_SESSION_DB ");
	}
	if ($persist) $ok = $ADODB_SESS_CONN->PConxxxt($ADODB_SESSION_CONxxxT,
			$ADODB_SESSION_USER,$ADODB_SESSION_PWD,$ADODB_SESSION_DB);
	else $ok = $ADODB_SESS_CONN->Conxxxt($ADODB_SESSION_CONxxxT,
			$ADODB_SESSION_USER,$ADODB_SESSION_PWD,$ADODB_SESSION_DB);
	
	if (!$ok) ADOConxxxtion::outp( "
-- Session: conxxxtion failed</p>",false);
}

/****************************************************************************************\
	Close the conxxxtion
\****************************************************************************************/
...
function adodb_sess_gc($maxlifetime) 
{
	...
	if ($ADODB_SESSION_EXPIRE_NOTIFY) {
		...
		if ($ADODB_SESS_DEBUG) ADOConxxxtion::outp("
