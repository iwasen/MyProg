...
// +----------------------------------------------------------------------+
//
// $Id: ADOdb.php,v 1.2 2005/11/22 15:47:29 ryu Exp $
//
...
class Auth_Container_ADOdb extends Auth_Container
{
     ...
     *
     * Initate conxxxtion to the database via PEAR::ADOdb
     *
     * @param  string Conxxxtion data or DB object
    ...
    function Auth_Container_ADOdb($dsn)
    {
        ...
        if (is_array($dsn)) {
            ...
            if (empty($this->options['dsn'])) {
                PEAR::raiseError('No conxxxtion parameters specified!');
            }
        } else {
    ...
    /**
     * Conxxxt to database by using the given DSN string
     *
     * @access private
     * @param  string DSN string
     * @return mixed  Object on error, otherwise bool
     */
     function _conxxxt($dsn)
    {
        if (is_string($dsn) || is_array($dsn)) {
        	if(!$this->db) {
	        	$this->db = &ADONewConxxxtion($dsn);
    ...
    /**
     * Prepare database conxxxtion
     *
     * This function checks if we have already opened a conxxxtion to
     * the database. If that's not the case, a new conxxxtion is opened.
     *
     * @access private
    ...
    {
    	if(!$this->db) {
    		$res = $this->_conxxxt($this->options['dsn']);  		
    	}
        return true;
    }
