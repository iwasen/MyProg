...
class adoSchema {
	...
	*/
	...
	/**
	* @var object	ADOdb conxxxtion object
	* @access private
	*/
	var $db;
	...
	* The only parameter is an ADOdb database conxxxtion object, which must already
	* have been created.
	*
	* @param object $db ADOdb database conxxxtion object.
	*/
	...
	* the filesystem and generate the SQL xxxessary to create the database described. 
	...
	* and generate the SQL xxxessary to create the database described by the schema.
	*
	* @param string $file Name of XML schema file.
	* @param bool $returnSchema Return schema rather than parsing.
	* @return array Array of SQL queries, ready to execute.
	...
	*
	* Call this method to load the specified schema (see the DTD for the proper format) from
	* the filesystem and generate the SQL xxxessary to remove the database described.
	...
	*
	* Call this method to parse a string containing an XML schema (see the DTD for the proper format)
	* and generate the SQL xxxessary to uninstall the database described by the schema. 
	* @see RemoveSchema()
	*
	...
	function addSQL( $sql = NULL ) {
		...
		if( is_string( $sql ) ) {
			...
			if( $this->ExecuteInline() && ( $this->success == 2 || $this->ContinueOnError() ) ) {
				...
				if( !$ok ) {
					if( $this->debug ) {
						ADOConxxxtion::outp( $this->db->ErrorMsg() );
					}
