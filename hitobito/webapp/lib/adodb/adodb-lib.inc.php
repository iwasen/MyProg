...
function _adodb_debug_execute(&$zthis, $sql, $inputarr)
{
	...
	*/
	...
	if ($inBrowser) {
		...
		if ($zthis->debug === -1)
			ADOConxxxtion::outp( "<br>\n($dbt): ".htmlspecialchars($sqlTxt)." &nbsp; <code>$ss</code>\n<br>\n",false);
		else 
			ADOConxxxtion::outp( "<hr>\n($dbt): ".htmlspecialchars($sqlTxt)." &nbsp; <code>$ss</code>\n<hr>\n",false);
	} else {
		ADOConxxxtion::outp("-----\n($dbt): ".$sqlTxt."\n-----\n",false);
	}
	...
	*/
	...
	// ErrorNo is a slow function call in mssql, and not reliable in PHP 4.0.6
		if($emsg = $zthis->ErrorMsg()) {
			if ($err = $zthis->ErrorNo()) ADOConxxxtion::outp($err.': '.$emsg);
		}
	} else if (!$qID) {
		ADOConxxxtion::outp($zthis->ErrorNo() .': '. $zthis->ErrorMsg());
	}
