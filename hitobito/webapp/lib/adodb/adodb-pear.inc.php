...
class DB
{
	...
	function &factory($type)
	{
		...
		$obj = &NewADOConxxxtion($type);
		if (!is_object($obj)) $obj =& new PEAR_Error('Unknown Database Driver: '.$dsninfo['phptype'],-1);
		return $obj;
	}

	/**
	 * Create a new DB object and conxxxt to the specified database
	 *
	 ...
	 * this conxxxtion should be persistent (for backends that support
	 * this).  This parameter can also be an array of options, see
	 * DB_common::setOption for more information on conxxxtion
	 * options.
	 *
	 * @return object a newly created DB conxxxtion object, or a DB
	 * error object on error
	 *
	 * @see DB::parseDSN
	 * @see DB::isError
	 */
	function &conxxxt($dsn, $options = false)
	{
		if (is_array($dsn)) {
			$dsninfo = $dsn;
		} else {
		...
		if($persist) $ok = $obj->PConxxxt($dsninfo['hostspec'], $dsninfo['username'],$dsninfo['password'],$dsninfo['database']);
		else  $ok = $obj->Conxxxt($dsninfo['hostspec'], $dsninfo['username'],$dsninfo['password'],$dsninfo['database']);
