...
class ADODB_ldap extends ADOConxxxtion {
    var $databaseType = 'ldap';
	var $dataProvider = 'ldap';
	
	# Conxxxtion information
    var $username = false;
    var $password = false;
	...
	function _conxxxt( $host, $username, $password, $ldapbase)
	{
	global $LDAP_CONxxxT_OPTIONS;
		
		if ( !function_exists( 'ldap_conxxxt' ) ) return null;
		...
		$this->_conxxxtionID = ldap_conxxxt( $conn_info[0], $conn_info[1] );
		if (!$this->_conxxxtionID) {
			$e = 'Could not conxxxt to ' . $conn_info[0];
			$this->_errorMsg = $e;
			if ($this->debug) ADOConxxxtion::outp($e);
			return false;
		}
		if( count( $LDAP_CONxxxT_OPTIONS ) > 0 ) {
			$this->_inject_bind_options( $LDAP_CONxxxT_OPTIONS );
		}
		
		if ($username) {
		    $bind = ldap_bind( $this->_conxxxtionID, $username, $password );
		} else {
			$username = 'anonymous';
		    $bind = ldap_bind( $this->_conxxxtionID );		
		}
...
/*
	...
	Make sure to set this BEFORE calling Conxxxt()

	Example:

	$LDAP_CONxxxT_OPTIONS = Array(
		Array (
...
*/
	...
	function _inject_bind_options( $options ) {
		foreach( $options as $option ) {
			ldap_set_option( $this->_conxxxtionID, $option["OPTION_NAME"], $option["OPTION_VALUE"] )
	...
	{
		$rs = ldap_search( $this->_conxxxtionID, $this->database, $sql );
		$this->_errorMsg = ($rs) ? '' : 'Search error on '.$sql;
		return $rs; 
	}

    /* closes the LDAP conxxxtion */
	function _close()
	{
		@ldap_close( $this->_conxxxtionID );
    ...
    function ServerInfo()
    {
        ...
        */
        ldap_get_option( $this->_conxxxtionID, LDAP_OPT_DEREF, $version['LDAP_OPT_DEREF'] ) ;
        ...
        */
        ldap_get_option( $this->_conxxxtionID, LDAP_OPT_SIZELIMIT, $version['LDAP_OPT_SIZELIMIT'] );
        ...
        Default: LDAP_NO_LIMIT
        */
        ldap_get_option( $this->_conxxxtionID, LDAP_OPT_TIMELIMIT, $version['LDAP_OPT_TIMELIMIT'] );
        ...
        Default: ON
        */
        ldap_get_option( $this->_conxxxtionID, LDAP_OPT_REFERRALS, $version['LDAP_OPT_REFERRALS'] );
        ...
        Default: OFF
        */
        ldap_get_option( $this->_conxxxtionID, LDAP_OPT_RESTART, $version['LDAP_OPT_RESTART'] );
        ...
        */
        ldap_get_option( $this->_conxxxtionID, LDAP_OPT_PROTOCOL_VERSION, $version['LDAP_OPT_PROTOCOL_VERSION'] );
        ...
        ldap_get_option( $this->_conxxxtionID, LDAP_OPT_HOST_NAME, $version['LDAP_OPT_HOST_NAME'] ); 
        ldap_get_option( $this->_conxxxtionID, LDAP_OPT_ERROR_NUMBER, $version['LDAP_OPT_ERROR_NUMBER'] ); 
        ldap_get_option( $this->_conxxxtionID, LDAP_OPT_ERROR_STRING, $version['LDAP_OPT_ERROR_STRING'] ); 
        ldap_get_option( $this->_conxxxtionID, LDAP_OPT_MATCHED_DN, $version['LDAP_OPT_MATCHED_DN'] ); 
...
class ADORecordSet_ldap extends ADORecordSet{	
	...
	function _initrs()
	{
	   ...
	   GetAssoc() function
       */
	    $this->_numOfRows = ldap_count_entries( $this->conxxxtion->_conxxxtionID, $this->_queryID );
	...
	*/
	...
	{		
        ...
        if ( $this->_currentRow == 0 ) {
		  $this->_entryID = ldap_first_entry( $this->conxxxtion->_conxxxtionID, $this->_queryID );
        } else {
          $this->_entryID = ldap_next_entry( $this->conxxxtion->_conxxxtionID, $this->_entryID );
        }
	    
	    $this->fields = ldap_get_attributes( $this->conxxxtion->_conxxxtionID, $this->_entryID );
