...
to conxxxt to the IBM DB2, we set the optional parameter
Cursortype when calling odbc_pconxxxt(....).
...
completely other location, so the whole conxxxtion
was made over a slow network conxxxtion).

I hope this helps when other encounter the same
problem when trying to conxxxt to DB2 from
PHP.
...
This conxxxtion attribute is defined by ODBC, but is not supported by DB2
...
$db = NewADOConxxxtion('db2');
$db->curMode = SQL_CUR_USE_ODBC;
$db->Conxxxt($dsn, $userid, $pwd);
...
Conxxxt() when using the CLI interface. From Halmai Csongor csongor.halmai#nexum.hu:

> The symptom is that if I change the database engine from postgres or any other to DB2 then the following
> conxxxtion command becomes wrong despite being described this version to be correct in the docs. 
>
> $conxxxtion_object->Conxxxt( $DATABASE_HOST, $DATABASE_AUTH_USER_NAME, $DATABASE_AUTH_PASSWORD, $DATABASE_NAME )
>
> In case of DB2 I had to swap the first and last arguments in order to conxxxt properly. 
...
class ADODB_DB2 extends ADODB_odbc {
	...
	function ServerInfo()
	{
		//odbc_setoption($this->_conxxxtionID,1,101 /*SQL_ATTR_ACCESS_MODE*/, 1 /*SQL_MODE_READ_ONLY*/);
		$vers = $this->GetOne('select versionnumber from sysibm.sysversions');
		//odbc_setoption($this->_conxxxtionID,1,101, 0 /*SQL_MODE_READ_WRITE*/);
	...
	global $ADODB_FETCH_MODE;
		...
		$qid = odbc_tables($this->_conxxxtionID, "", $qschema, $qtable, "");
	...
	function &SelectLimit($sql,$nrows=-1,$offset=-1,$inputArr=false)
	{
		...
		// could also use " OPTIMIZE FOR $nrows ROWS "
			...
			$rs =& ADOConxxxtion::SelectLimit($sql,-1,$offset,$inputArr);
		}
