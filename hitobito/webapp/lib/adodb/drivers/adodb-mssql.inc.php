...
// just after you conxxxt to the database. Supports mdy and dmy only.
...
global $ADODB_mssql_date_order;
	...
	if ($adate) {
		...
		if ($anum > 0) {
			if ($anum > 31) {
				//ADOConxxxtion::outp( "MSSQL: YYYY-MM-DD date format not supported currently");
			} else
...
class ADODB_mssql extends ADOConxxxtion {
...
order by constraint_name, referenced_table_name, keyno";
	...
	function ErrorNo() 
	{
		...
		$id = @mssql_query("select @@ERROR",$this->_conxxxtionID);
		if (!$id) return false;
	...
	{
		if (!function_exists('mssql_pconxxxt')) return null;
		$this->_conxxxtionID = mssql_conxxxt($argHostname,$argUsername,$argPassword);
	...
	{
		if (!function_exists('mssql_pconxxxt')) return null;
		$this->_conxxxtionID = mssql_pconxxxt($argHostname,$argUsername,$argPassword);
		if ($this->_conxxxtionID === false) return false;
		
		// persistent conxxxtions can forget to rollback on crash, so we do it here.
	...
	function PrepareSP($sql)
	{
		if (!$this->_has_mssql_init) {
			ADOConxxxtion::outp( "PrepareSP: mssql_init only available since PHP 4.1.0");
			return $sql;
		}
		$stmt = mssql_init($sql,$this->_conxxxtionID);
		if (!$stmt)  return $sql;
	...
	*/
	...
	{
		if (!$this->_has_mssql_init) {
			ADOConxxxtion::outp( "Parameter: mssql_bind only available since PHP 4.1.0");
			return $sql;
		}
	...
	*/
	...
	function _query($sql,$inputarr)
	{
		...
		if (is_array($inputarr)) {
			...
			$decl = $this->qstr($decl);
			if ($this->debug) ADOConxxxtion::outp("<font size=-1>sp_executesql N{$sql[1]},N$decl,$params</font>");
			...
			$rez = mssql_query($sql,$this->_conxxxtionID);
		}
		return $rez;
	}
	...
	{ 
		...
		$rez = @mssql_close($this->_conxxxtionID);
