...
class ADODB_fbsql extends ADOConxxxtion {
	...
	function _insertid()
	{
			return fbsql_insert_id($this->_conxxxtionID);
	}
	
	function _affectedrows()
	{
			return fbsql_affected_rows($this->_conxxxtionID);
	}
  
  	function &MetaDatabases()
	{
		$qid = fbsql_list_dbs($this->_conxxxtionID);
		$arr = array();
		$i = 0;
	...
	{
		$this->_conxxxtionID = fbsql_conxxxt($argHostname,$argUsername,$argPassword);
	...
	{
		$this->_conxxxtionID = fbsql_pconxxxt($argHostname,$argUsername,$argPassword);
		if ($this->_conxxxtionID === false) return false;
		if ($argDatabasename) return $this->SelectDB($argDatabasename);
		return true;	
	}
	...
	function SelectDB($dbName) 
	{
		...
		if ($this->_conxxxtionID) {
			return @fbsql_select_db($dbName,$this->_conxxxtionID);		
		}
		else return false;	
	}
	...
	{
		return fbsql_query("$sql;",$this->_conxxxtionID);
	}

	/*	Returns: the last error message from previous database operation	*/	
	function ErrorMsg() 
	{
		$this->_errorMsg = @fbsql_error($this->_conxxxtionID);
	...
	function ErrorNo() 
	{
		return @fbsql_errno($this->_conxxxtionID);
	}
	
	// returns true or false
	function _close()
	{
		return @fbsql_close($this->_conxxxtionID);
	}
