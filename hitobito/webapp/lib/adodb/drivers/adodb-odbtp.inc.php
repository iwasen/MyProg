...
class ADODB_odbtp extends ADOConxxxtion{
	...
	function ServerInfo()
	{
		return array('description' => @odbtp_get_attr( ODB_ATTR_DBMSNAME, $this->_conxxxtionID),
		             'version' => @odbtp_get_attr( ODB_ATTR_DBMSVER, $this->_conxxxtionID));
	}

	function ErrorMsg()
	{
		if (empty($this->_conxxxtionID)) return @odbtp_last_error();
		return @odbtp_last_error($this->_conxxxtionID);
	}

	function ErrorNo()
	{
		if (empty($this->_conxxxtionID)) return @odbtp_last_error_state();
			return @odbtp_last_error_state($this->_conxxxtionID);
	}
	...
	function CreateSequence($seqname='adodbseq',$start=1)
	{
		...
		if( $this->odbc_driver == ODB_DRIVER_FOXPRO ) {
			$path = @odbtp_get_attr( ODB_ATTR_DATABASENAME, $this->_conxxxtionID );
    ...
    function _conxxxt($HostOrInterface, $UserOrDSN='', $argPassword='', $argDatabase='')
	{
		$this->_conxxxtionID = @odbtp_conxxxt($HostOrInterface,$UserOrDSN,$argPassword,$argDatabase);
		if ($this->_conxxxtionID === false) {
			$this->_errorMsg = $this->ErrorMsg() ;
			return false;
		}
		if ($this->_dontPoolDBC) {
			if (function_exists('odbtp_dont_pool_dbc'))
				@odbtp_dont_pool_dbc($this->_conxxxtionID);
		}
		else {
			$this->_dontPoolDBC = true;
		}
		$this->odbc_driver = @odbtp_get_attr(ODB_ATTR_DRIVER, $this->_conxxxtionID);
		$dbms = strtolower(@odbtp_get_attr(ODB_ATTR_DBMSNAME, $this->_conxxxtionID));
		$this->odbc_name = $dbms;
		...
		switch( $dbms ) {
			...
			default:
				...
				if( @odbtp_get_attr(ODB_ATTR_TXNCAPABLE, $this->_conxxxtionID) )
					$this->hasTransactions = true;
				else
					$this->hasTransactions = false;
		}
        @odbtp_set_attr(ODB_ATTR_FULLCOLINFO, TRUE, $this->_conxxxtionID );

		if ($this->_useUnicodeSQL )
			@odbtp_set_attr(ODB_ATTR_UNICODESQL, TRUE, $this->_conxxxtionID);

        return true;
	}

	function _pconxxxt($HostOrInterface, $UserOrDSN='', $argPassword='', $argDatabase='')
	{
		$this->_dontPoolDBC = false;
  		return $this->_conxxxt($HostOrInterface, $UserOrDSN, $argPassword, $argDatabase);
	}

	function SelectDB($dbName)
	{
		if (!@odbtp_select_db($dbName, $this->_conxxxtionID)) {
			return false;
		}
	...
	function BeginTrans()
	{
		...
		$rs = @odbtp_set_attr(ODB_ATTR_TRANSACTIONS,$txn,$this->_conxxxtionID);
		if(!$rs) return false;
		return true;
	}
	...
	function CommitTrans($ok=true)
	{
		...
		if( ($ret = @odbtp_commit($this->_conxxxtionID)) )
			$ret = @odbtp_set_attr(ODB_ATTR_TRANSACTIONS, ODB_TXN_NONE, $this->_conxxxtionID);//set transaction off
		return $ret;
	}
	...
	function RollbackTrans()
	{
		...
		if ($this->transCnt) $this->transCnt -= 1;
		$this->autoCommit = true;
		if( ($ret = @odbtp_rollback($this->_conxxxtionID)) )
			$ret = @odbtp_set_attr(ODB_ATTR_TRANSACTIONS, ODB_TXN_NONE, $this->_conxxxtionID);//set transaction off
		return $ret;
	}
	...
	function &SelectLimit($sql,$nrows=-1,$offset=-1, $inputarr=false,$secs2cache=0)
	{
		...
		$ret =& ADOConxxxtion::SelectLimit($sql,$nrows,$offset,$inputarr,$secs2cache);
		return $ret;
	}
	...
	function Prepare($sql)
	{
		...
		$stmt = @odbtp_prepare($sql,$this->_conxxxtionID);
		if (!$stmt) {
	...
	function PrepareSP($sql)
	{
		...
		$stmt = @odbtp_prepare_proc($sql,$this->_conxxxtionID);
	...
	*/
	...
	function UpdateBlob($table,$column,$val,$where,$blobtype='image')
	{
		...
		if( !($stmt = @odbtp_prepare($sql, $this->_conxxxtionID)) )
			return false;
	...
	function _query($sql,$inputarr=false)
	{
 		if ($inputarr) {
			if (is_array($sql)) {
				...
				$stmtid = @odbtp_prepare($sql,$this->_conxxxtionID);
				if ($stmtid == false) {
			...
			$stmtid = @odbtp_query($sql,$this->_conxxxtionID);
   		}
	...
	function _close()
	{
		$ret = @odbtp_close($this->_conxxxtionID);
...
class ADORecordSet_odbtp extends ADORecordSet {
	...
	function _initrs()
	{
		...
		if (!$this->conxxxtion->_useUnicodeSQL) return;

		if ($this->conxxxtion->odbc_driver == ODB_DRIVER_JET) {
			if (!@odbtp_get_attr(ODB_ATTR_MAPCHARTOWCHAR,
			                     $this->conxxxtion->_conxxxtionID))
			{
