...
/*
 V4.65 22 July 2005  (c) 2000-2005 John Lim (jlim@natsoft.com.my). All rights reserved.
  ...
  27 Nov 2000 jlim - added changes to _conxxxt/_pconxxxt from ideas by "Lennie" <leen@wirehub.nl>
...
class ADODB_postgres64 extends ADOConxxxtion{
...
WHERE relkind in ('r','v') AND (c.relname='%s' or c.relname = lower('%s'))
 and c.relnamespace=n.oid and n.nspname='%s' 
	...
	{
		$result=pg_exec($this->_conxxxtionID, "SELECT last_value FROM ${tablename}_${fieldname}_seq");
		if ($result) {
...
// Warning: This compilation does not support pg_cmdtuples() in d:/inetpub/wwwroot/php/adodb/adodb-postgres.inc.php on line 44
   function _affectedrows()
   {
	...
	function BeginTrans()
	{
		...
		$this->transCnt += 1;
		return @pg_Exec($this->_conxxxtionID, "begin");
	}
	...
	{ 
		...
		$this->transCnt -= 1;
		return @pg_Exec($this->_conxxxtionID, "commit");
	}
	...
	function RollbackTrans()
	{
		if ($this->transOff) return true;
		$this->transCnt -= 1;
		return @pg_Exec($this->_conxxxtionID, "rollback");
	}
...
select viewname,'V' from pg_views where viewname like $mask";
	...
	*/ 
	...
	{ 
		pg_exec ($this->_conxxxtionID, "begin"); 
		
		$fd = fopen($path,'r');
		$contents = fread($fd,filesize($path));
		fclose($fd);
		
		$oid = pg_lo_create($this->_conxxxtionID);
		$handle = pg_lo_open($this->_conxxxtionID, $oid, 'w');
		pg_lo_write($handle, $contents);
		pg_lo_close($handle);
		
		// $oid = pg_lo_import ($path); 
		pg_exec($this->_conxxxtionID, "commit"); 
		$rs = ADOConxxxtion::UpdateBlob($table,$column,$oid,$where,$blobtype); 
		$rez = !empty($rs); 
		return $rez; 
	} 
	...
	*/ 
	function BlobDecode($blob,$maxsize=false,$hastrans=true) 
	{
		...
		if ($hastrans) @pg_exec($this->_conxxxtionID,"begin"); 
		$fd = @pg_lo_open($this->_conxxxtionID,$blob,"r");
		if ($fd === false) {
			if ($hastrans) @pg_exec($this->_conxxxtionID,"commit");
			return $blob;
		}
...
WHERE c2.relname=\'%s\' or c2.relname=lower(\'%s\')';
	...
	//
	// examples:
	// 	$db->Conxxxt("host=host1 user=user1 password=secret port=4341");
	// 	$db->Conxxxt('host1','user1','secret');
	function _conxxxt($str,$user='',$pwd='',$db='',$ctype=0)
	{
		
		if (!function_exists('pg_pconxxxt')) return null;
		...
		if ($ctype === 1) { // persistent
			$this->_conxxxtionID = pg_pconxxxt($str);
		} else {
			if ($ctype === -1) { // nconxxxt, we trick pgsql ext by changing the conxxxtion str
			static $ncnt;
			...
			$this->_conxxxtionID = pg_conxxxt($str);
		}
		if ($this->_conxxxtionID === false) return false;
	...
	//
	// examples:
	// 	$db->PConxxxt("host=host1 user=user1 password=secret port=4341");
	// 	$db->PConxxxt('host1','user1','secret');
	function _pconxxxt($str,$user='',$pwd='',$db='')
	{
		return $this->_conxxxt($str,$user,$pwd,$db,1);
	}
	...
	function _query($sql,$inputarr)
	{
		...
		*/
			...
			$rez = @pg_exec($this->_conxxxtionID,$exsql);
			if (!$rez) {
			...
			$this->_errorMsg = false;
			//adodb_backtrace();
			$rez = pg_exec($this->_conxxxtionID,$sql);
		}
	...
	function ErrorMsg() 
	{
		...
		if (ADODB_PHPVER >= 0x4300) {
			...
			if (!empty($this->_conxxxtionID)) {
				$this->_errorMsg = @pg_last_error($this->_conxxxtionID);
			} else $this->_errorMsg = @pg_last_error();
		} else {
			if (empty($this->_conxxxtionID)) $this->_errorMsg = @pg_errormessage();
			else $this->_errorMsg = @pg_errormessage($this->_conxxxtionID);
		}
	...
	function ErrorNo()
	{
		$e = $this->ErrorMsg();
		if (strlen($e)) {
			return ADOConxxxtion::MetaError($e);
		 }
		 return 0;
	}
...
class ADORecordSet_postgres64 extends ADORecordSet{
	...
	global $ADODB_COUNTRECS;
		...
		if (empty($this->conxxxtion->noBlobs))
