...
class ADODB_oracle extends ADOConxxxtion {
	...
	{	  
		 ...
		 ora_commitoff($this->_conxxxtionID);
		 return true;
	}
	...
	{ 
		   ...
		   $ret = ora_commit($this->_conxxxtionID);
		   ora_commiton($this->_conxxxtionID);
		   return $ret;
	}
	...
	{
		$ret = ora_rollback($this->_conxxxtionID);
		ora_commiton($this->_conxxxtionID);
		return $ret;
	}
	...
	function ErrorMsg() 
 	{   
        ...
        if (is_resource($this->_curs)) $this->_errorMsg = @ora_error($this->_curs);
 		if (empty($this->_errorMsg)) $this->_errorMsg = @ora_error($this->_conxxxtionID);
	...
	function ErrorNo() 
	{
		...
		if (empty($this->_errorCode)) $this->_errorCode = @ora_errorcode($this->_conxxxtionID);
		...
		function _conxxxt($argHostname, $argUsername, $argPassword, $argDatabasename, $mode=0)
		{
            ...
            // <G. Giunta 2003/03/03/> Reset error messages before conxxxting
...
//			if ($argHostname) putenv("ORACLE_HOME=$argHostname");
		...
		//if ($argHostname) print "<p>Conxxxt: 1st argument should be left blank for $this->databaseType</p>";
			if ($mode = 1)
				$this->_conxxxtionID = ora_plogon($argUsername,$argPassword);
			else
				$this->_conxxxtionID = ora_logon($argUsername,$argPassword);
			if ($this->_conxxxtionID === false) return false;
			if ($this->autoCommit) ora_commiton($this->_conxxxtionID);
			if ($this->_initdate) {
		...
		function _query($sql,$inputarr=false)
		{
			...
			$curs = ora_open($this->_conxxxtionID);
            ...
            // that we can obtain ONLY from the cursor (and not from the conxxxtion)
		...
		function _close()
		{
			return @ora_logoff($this->_conxxxtionID);
		}
