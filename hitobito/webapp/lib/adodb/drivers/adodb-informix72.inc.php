...
class ADODB_informix72 extends ADOConxxxtion {
	...
	var $_bindInputArray = true;  // set to true if ADOConxxxtion.Execute() permits binding of array parameters.
	var $sysDate = 'TODAY';
...
/*  //!eos.
   ...
   {
		return ADOConxxxtion::MetaColumns($table,false);
   }
   ...
   function _pconxxxt($argHostname, $argUsername, $argPassword, $argDatabasename)
	{
		...
		$dbs = $argDatabasename . "@" . $argHostname;
		putenv("INFORMIXSERVER=".trim($argHostname)); 
		$this->_conxxxtionID = ifx_pconxxxt($dbs,$argUsername,$argPassword);
		if ($this->_conxxxtionID === false) return false;
		#if ($argDatabasename) return $this->SelectDB($argDatabasename);
		return true;
	}
...
*/
	...
	global $ADODB_COUNTRECS;
	  ...
	  if (!$ADODB_COUNTRECS && preg_match("/^\s*select/is", $sql)) {
		 if ($inputarr) {
			$this->lastQuery = ifx_query($sql,$this->_conxxxtionID, $this->cursorType, $tab);
		 }
		 else {
			$this->lastQuery = ifx_query($sql,$this->_conxxxtionID, $this->cursorType);
		 }
	  }
	  else {
		 if ($inputarr) {
			$this->lastQuery = ifx_query($sql,$this->_conxxxtionID, $tab);
		 }
		 else {
			$this->lastQuery = ifx_query($sql,$this->_conxxxtionID);
		 }
	  }
	  ...
	  //if ($this->_autocommit) ifx_query('COMMIT',$this->_conxxxtionID);
	...
	function _close()
	{
		...
		return ifx_close($this->_conxxxtionID);
	}
}
