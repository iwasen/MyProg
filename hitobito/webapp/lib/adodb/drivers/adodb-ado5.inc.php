...
class ADODB_ado extends ADOConxxxtion {
	...
	function ServerInfo()
	{
		if (!empty($this->_conxxxtionID)) $desc = $this->_conxxxtionID->provider;
	...
	// you can also pass a conxxxtion string like this:
	//
	// $DB->Conxxxt('USER ID=sa;PASSWORD=pwd;SERVER=mangrove;DATABASE=ai',false,false,'SQLOLEDB');
	function _conxxxt($argHostname, $argUsername, $argPassword, $argProvider= 'MSDASQL')
	{
		try {
		$u = 'UID';
		$p = 'PWD';
		...
		if (!empty($this->charPage))
			$dbc = new COM('ADODB.Conxxxtion',null,$this->charPage);
		else
			$dbc = new COM('ADODB.Conxxxtion');
		...
		if ($argProvider=='mssql') {
			...
			//use trusted coxxxtion for SQL if username not specified
			if (!$argUsername) $argHostname .= ";Trusted_Conxxxtion=Yes";
		...
		if ($this->debug) ADOConxxxtion::outp( "Host=".$argHostname."<BR>\n version=$dbc->version");
		// @ added below for php 4.0.1 and earlier
		@$dbc->Open((string) $argHostname);
		
		$this->_conxxxtionID = $dbc;
	...
	function _pconxxxt($argHostname, $argUsername, $argPassword, $argProvider='MSDASQL')
	{
		return $this->_conxxxt($argHostname,$argUsername,$argPassword,$argProvider);
	}	
...
*/
	...
	{ 
		...
		if (isset($this->_thisTransactions))
			...
			$o = $this->_conxxxtionID->Properties("Transaction DDL");
			$this->_thisTransactions = $o ? true : false;
			if (!$o) return false;
		}
		@$this->_conxxxtionID->BeginTrans();
		$this->transCnt += 1;
		return true;
	}
	...
	{ 
		...
		@$this->_conxxxtionID->CommitTrans();
		if ($this->transCnt) @$this->transCnt -= 1;
		return true;
	}
	function RollbackTrans() {
		if ($this->transOff) return true;
		@$this->_conxxxtionID->RollbackTrans();
		if ($this->transCnt) @$this->transCnt -= 1;
		return true;
	}
	...
	function ErrorMsg() 
	{
		$errc = $this->_conxxxtionID->Errors;
	...
	function _close()
	{
		if ($this->_conxxxtionID) $this->_conxxxtionID->Close();
	...
	{	
		...
		for ($i=0,$max = $this->_numOfFields; $i < $max; $i++) {
			...
			case 6: // currency is not supported properly;
				ADOConxxxtion::outp( '<b>'.$f->Name.': currency type not supported by PHP</b>');
