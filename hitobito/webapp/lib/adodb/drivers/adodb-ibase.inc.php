...
V4.65 22 July 2005  (c) 2000-2005 John Lim (jlim@natsoft.com.my). All rights reserved.  
  ...
  Docs to interbase at the website
   http://www.syxxxtics.co.za/php3/tutorial/IB_PHP3_API.html
...
class ADODB_ibase extends ADOConxxxtion {
	...
	function _conxxxt($argHostname, $argUsername, $argPassword, $argDatabasename,$persist=false)
	{  
		if (!function_exists('ibase_pconxxxt')) return null;
		if ($argDatabasename) $argHostname .= ':'.$argDatabasename;
		$fn = ($persist) ? 'ibase_pconxxxt':'ibase_conxxxt';
		if ($this->role)
			$this->_conxxxtionID = $fn($argHostname,$argUsername,$argPassword,
	...
	function ServerInfo()
	{
		...
		$arr['version'] = ADOConxxxtion::_findvers($s);
		$arr['description'] = $s;
		return $arr;
	}
	...
	{	 
		...
		$this->autoCommit = false;
	 	$this->_transactionID = $this->_conxxxtionID;//ibase_trans($this->ibasetrans, $this->_conxxxtionID);
	...
	global $ADODB_COUNTRECS;
		...
		if ($this->_logsql) {
			...
			$ret =& ADOConxxxtion::_Execute($sql,$inputarr);
	...
	{
		$stmt = ibase_prepare($this->_conxxxtionID,$sql);
	...
	{ 
		...
		if (is_array($sql)) {
			...
			if (is_array($iarr)) {
				if  (ADODB_PHPVER >= 0x4050) { // actually 4.0.4
					...
					default: ADOConxxxtion::outp( "Too many parameters to ibase query $sql");
		...
		if ($docommit && $ret === true) ibase_commit($this->_conxxxtionID);
	 ...
	 function _close()
	 {	   
		if (!$this->autoCommit) @ibase_rollback($this->_conxxxtionID);
		return @ibase_close($this->_conxxxtionID);
	 }
	...
	{
		$blobid = ibase_blob_create( $this->_conxxxtionID);
	...
	{ 
		...
		$blob_id = ibase_blob_create($this->_conxxxtionID); 
	...
	function _fetch() 
	{
		...
		for ($i=0, $max = $this->_numOfFields; $i < $max; $i++) { 
			if ($this->_cacheType[$i]=="BLOB") {
				if (isset($f[$i])) { 
					$f[$i] = $this->conxxxtion->_BlobDecode($f[$i]); 
				} else { 
					$f[$i] = null; 
				} 
			} else { 
