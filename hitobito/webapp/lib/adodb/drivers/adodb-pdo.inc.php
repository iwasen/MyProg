...
class ADODB_pdo_base extends ADODB_pdo {
	...
	function _init($parentDriver)
	{
		...
		#$parentDriver->_conxxxtionID->setAttribute(PDO_MYSQL_ATTR_USE_BUFFERED_QUERY,true);
	}
...
class ADODB_pdo extends ADOConxxxtion {
	...
	function _conxxxt($argDSN, $argUsername, $argPassword, $argDatabasename, $persist=false)
	{
		$at = strpos($argDSN,':');
		$this->dsnType = substr($argDSN,0,$at);

		try {
			$this->_conxxxtionID = new PDO($argDSN, $argUsername, $argPassword);
		} catch (Exception $e) {
			$this->_conxxxtionID = false;
			$this->_errorno = -1;
			//var_dump($e);
			$this->_errormsg = 'Conxxxtion attempt failed: '.$e->getMessage();
			return false;
		}
		...
		if ($this->_conxxxtionID) {
			...
			//$this->_conxxxtionID->setAttribute(PDO_ATTR_ERRMODE,PDO_ERRMODE_SILENT );
			$this->_conxxxtionID->setAttribute(PDO_ATTR_CASE,$m);
			
			$class = 'ADODB_pdo_'.$this->dsnType;
			//$this->_conxxxtionID->setAttribute(PDO_ATTR_AUTOCOMMIT,true);
			switch($this->dsnType) {
			case 'oci':
			case 'mysql':
			case 'pgsql':
			...
			$this->_driver->_conxxxtionID = $this->_conxxxtionID;
			$this->_UpdatePDO();
			return true;
		}
		$this->_driver = new ADODB_pdo_base();
	...
	{
		return $this->_conxxxt($argDSN, $argUsername, $argPassword, $argDatabasename, true);
	}
	...
	function ErrorMsg()
	{
		...
		else if (!empty($this->_conxxxtionID)) $arr = $this->_conxxxtionID->errorInfo();
		else return 'No Conxxxtion Established';
	...
	function ErrorNo()
	{
		...
		else if (!empty($this->_conxxxtionID)) {
			$arr = $this->_conxxxtionID->errorInfo();
	...
	{	
		...
		$this->_conxxxtionID->setAttribute(PDO_ATTR_AUTOCOMMIT,false);
		return $this->_conxxxtionID->beginTransaction();
	}
	...
	function RollbackTrans()
	{
		...
		$ret = $this->_conxxxtionID->rollback();
		$this->_conxxxtionID->setAttribute(PDO_ATTR_AUTOCOMMIT,true);
		return $ret;
	}
	...
	{
		$this->_stmt = $this->_conxxxtionID->prepare($sql);
	...
	function _insertid()
	{
		return ($this->_conxxxtionID) ? $this->_conxxxtionID->lastInsertId() : 0;
	}
}
...
class ADOPDOStatement {
	...
	function ADOPDOStatement($stmt,$conxxxtion)
	{
		$this->_stmt = $stmt;
		$this->_conxxxtionID = $conxxxtion;
	}
	
	function Execute($inputArr=false)
	{
		$savestmt = $this->_conxxxtionID->_stmt;
		$rs = $this->_conxxxtionID->Execute(array(false,$this->_stmt),$inputArr);
		$this->_conxxxtionID->_stmt = $savestmt;
		return $rs;
	}
	...
	function ErrorMsg()
	{
		...
		else $arr = $this->_conxxxtionID->errorInfo();
	...
	function ErrorNo()
	{
		...
		else return $this->_conxxxtionID->errorInfo();
	}
}
