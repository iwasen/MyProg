...
class ADODB_sqlite extends ADOConxxxtion {
...
/*
  function __get($name) 
  {
	...
	function _insertid()
	{
		return sqlite_last_insert_rowid($this->_conxxxtionID);
	}
	
	function _affectedrows()
	{
        return sqlite_changes($this->_conxxxtionID);
    }
	...
	{
		@sqlite_create_function($this->_conxxxtionID, 'adodb_date', 'adodb_date', 1);
		@sqlite_create_function($this->_conxxxtionID, 'adodb_date2', 'adodb_date2', 2);
	}
	...
	function _conxxxt($argHostname, $argUsername, $argPassword, $argDatabasename)
	{
		...
		$this->_conxxxtionID = sqlite_open($argHostname);
	...
	function _pconxxxt($argHostname, $argUsername, $argPassword, $argDatabasename)
	{
		...
		$this->_conxxxtionID = sqlite_popen($argHostname);
		if ($this->_conxxxtionID === false) return false;
		$this->_createFunctions();
		return true;
	}
	...
	{
		$rez = sqlite_query($sql,$this->_conxxxtionID);
		if (!$rez) {
			$this->_errorNo = sqlite_last_error($this->_conxxxtionID);
		}
	...
	*/
	...
	function _close()
	{
		return @sqlite_close($this->_conxxxtionID);
	}
