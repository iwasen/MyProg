...
class ADODB_sybase extends ADOConxxxtion {
	...
	*/
	...
	{
		if (!function_exists('sybase_conxxxt')) return null;
		
		$this->_conxxxtionID = sybase_conxxxt($argHostname,$argUsername,$argPassword);
	...
	function _pconxxxt($argHostname, $argUsername, $argPassword, $argDatabasename)
	{
		...
		$this->_conxxxtionID = sybase_pconxxxt($argHostname,$argUsername,$argPassword);
		if ($this->_conxxxtionID === false) return false;
		if ($argDatabasename) return $this->SelectDB($argDatabasename);
		return true;	
	}
	...
	global $ADODB_COUNTRECS;
		...
		if ($ADODB_COUNTRECS == false && ADODB_PHPVER >= 0x4300)
			return sybase_unbuffered_query($sql,$this->_conxxxtionID);
		else
			return sybase_query($sql,$this->_conxxxtionID);
	}
	...
	function &SelectLimit($sql,$nrows=-1,$offset=-1,$inputarr=false,$secs2cache=0) 
	{
		...
		$rs =& ADOConxxxtion::SelectLimit($sql,$nrows,$offset,$inputarr,0);
	...
	function _close()
	{ 
		return @sybase_close($this->_conxxxtionID);
	}
