...
class ADODB_oci8 extends ADOConxxxtion {
...
/*
  ...
  Multiple modes of conxxxtion are supported:
  
  a. Local Database
    $conn->Conxxxt(false,'scott','tiger');
  
  b. From tnsnames.ora
    $conn->Conxxxt(false,'scott','tiger',$tnsname); 
    $conn->Conxxxt($tnsname,'scott','tiger'); 
  
  c. Server + service name
    $conn->Conxxxt($serveraddress,'scott,'tiger',$service_name);
  
  d. Server + SID
  	$conn->conxxxtSID = true;
	$conn->Conxxxt($serveraddress,'scott,'tiger',$SID);
...
NATSOFT.DOMAIN =
  ...
  There are 3 conxxxtion modes, 0 = non-persistent, 1 = persistent, 2 = force new conxxxtion
	
*/
	function _conxxxt($argHostname, $argUsername, $argPassword, $argDatabasename,$mode=0)
	{
		...
		if($argHostname) { // added by Jorma Tuomainen <jorma.tuomainen@ppoy.fi>
			if (empty($argDatabasename)) $argDatabasename = $argHostname;
				...
				if ($this->conxxxtSID) {
					...
					.")(PORT=$argHostport))(CONxxxT_DATA=(SID=$argDatabasename)))";
				} else
					$argDatabasename="(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=".$argHostname
					.")(PORT=$argHostport))(CONxxxT_DATA=(SERVICE_NAME=$argDatabasename)))";
			}
		}
				
 		//if ($argHostname) print "<p>Conxxxt: 1st argument should be left blank for $this->databaseType</p>";
		if ($mode==1) {
			$this->_conxxxtionID = ($this->charSet) ? 
				OCIPLogon($argUsername,$argPassword, $argDatabasename)
				:
				OCIPLogon($argUsername,$argPassword, $argDatabasename, $this->charSet)
				;
			if ($this->_conxxxtionID && $this->autoRollback)  OCIrollback($this->_conxxxtionID);
		} else if ($mode==2) {
			$this->_conxxxtionID = ($this->charSet) ? 
		...
		if (!$this->_conxxxtionID) return false;
		if ($this->_initdate) {
		...
		// $vers = OCIServerVersion($this->_conxxxtionID);
	...
	function ServerInfo()
	{
		...
		$arr['description'] = @OCIServerVersion($this->_conxxxtionID);
		$arr['version'] = ADOConxxxtion::_findvers($arr['description']);
		return $arr;
	}
		// returns true or false
	function _pconxxxt($argHostname, $argUsername, $argPassword, $argDatabasename)
	{
		return $this->_conxxxt($argHostname, $argUsername, $argPassword, $argDatabasename,1);
	}
	
	// returns true or false
	function _nconxxxt($argHostname, $argUsername, $argPassword, $argDatabasename)
	{
		return $this->_conxxxt($argHostname, $argUsername, $argPassword, $argDatabasename,2);
	}
	...
	function &MetaTables($ttype=false,$showSchema=false,$mask=false) 
	{
		...
		$ret =& ADOConxxxtion::MetaTables($ttype,$showSchema);
	...
	{ 
		...
		$ret = OCIcommit($this->_conxxxtionID);
	...
	function RollbackTrans()
	{
		if ($this->transOff) return true;
		if ($this->transCnt) $this->transCnt -= 1;
		$ret = OCIrollback($this->_conxxxtionID);
		$this->_commit = OCI_COMMIT_ON_SUCCESS;
		$this->autoCommit = true;
		return $ret;
	}
	...
	function ErrorMsg() 
	{
		...
		if (empty($arr)) {
			$arr = @OCIerror($this->_conxxxtionID);
	...
	*/
	function &SelectLimit($sql,$nrows=-1,$offset=-1, $inputarr=false,$secs2cache=0)
	{
		...
		if ($offset < $this->selectOffsetAlg1) {
			...
			$rs =& ADOConxxxtion::SelectLimit($sql,$nrows,$offset,$inputarr,$secs2cache);
			return $rs;
	...
	*/
	...
	function UpdateBlob($table,$column,$val,$where,$blobtype='BLOB')
	{
		...
		default: ADOConxxxtion::outp("<b>UpdateBlob</b>: Unknown blobtype=$blobtype"); return false;
		...
		$desc = OCINewDescriptor($this->_conxxxtionID, OCI_D_LOB);
	...
	*/
	...
	static $BINDNUM = 0;
		...
		$stmt = OCIParse($this->_conxxxtionID,$sql);
		...
		if ($sttype == 'BEGIN' || $sttype == 'DECLARE') {
			return array($sql,$stmt,0,$BINDNUM, ($cursor) ? OCINewCursor($this->_conxxxtionID) : false);
		}
	...
	*/
	function Bind(&$stmt,&$var,$size=4000,$type=false,$name=false,$isOutput=false)
	{
		...
		if ($name == false) {
			...
			if ($this->debug) {
				ADOConxxxtion::outp("<b>Bind</b>: name = $name");
			}
            //we have to create a new Descriptor here
			$numlob = count($this->_refLOBs);
        	$this->_refLOBs[$numlob]['LOB'] = OCINewDescriptor($this->_conxxxtionID, oci_lob_desc($type));
			$this->_refLOBs[$numlob]['TYPE'] = $isOutput;
			
			$tmp = &$this->_refLOBs[$numlob]['LOB'];
	        $rez = OCIBindByName($stmt[1], ":".$name, $tmp, -1, $type);
			if ($this->debug) {
				ADOConxxxtion::outp("<b>Bind</b>: descriptor has been allocated, var (".$name.") binded");
			}
			...
			if ($isOutput == false) {
				...
				if ($this->debug) {
					ADOConxxxtion::outp("<b>Bind</b>: LOB has been written to temp");
				}
			} else {
				$this->_refLOBs[$numlob]['VAR'] = &$var;
			}
	...
	*/
	function Parameter(&$stmt,&$var,$name,$isOutput=false,$maxLen=4000,$type=false)
	{
			if  ($this->debug) {
				...
				ADOConxxxtion::outp( "{$prefix}Parameter(\$stmt, \$php_var='$var', \$name='$name', \$maxLen=$maxLen, \$type=$ztype);");
			}
...
//OCIInternalDebug(1);			
			if (count($this -> _refLOBs) > 0) {
				...
				foreach ($this -> _refLOBs as $key => $value) {
					if ($this -> _refLOBs[$key]['TYPE'] == true) {
						...
						if ($this -> debug) {
							ADOConxxxtion::outp("<b>OUT LOB</b>: LOB has been loaded. <br>");
						}
                        ...
                        if ($this->debug) {
							ADOConxxxtion::outp("<b>IN LOB</b>: LOB has been saved. <br>");
						}
                    }					
				}
			}
	...
	function _close()
	{
		if (!$this->_conxxxtionID) return;
		
		if (!$this->autoCommit) OCIRollback($this->_conxxxtionID);
		if (count($this->_refLOBs) > 0) {
			foreach ($this ->_refLOBs as $key => $value) {
				$this->_refLOBs[$key]['LOB']->free();
				unset($this->_refLOBs[$key]);
...
class ADORecordset_oci8 extends ADORecordSet {
	...
	function Init()
	{
		...
		if ($this->_queryID) {
			...
			if ($err && $this->conxxxtion->debug) ADOConxxxtion::outp($err);
			*/
	...
	function _close() 
	{
		if ($this->conxxxtion->_stmt === $this->_queryID) $this->conxxxtion->_stmt = false;
	...
	function MetaType($t,$len=-1)
	{
		...
		case 'DATE': 
			return  ($this->conxxxtion->datetime) ? 'T' : 'D';
