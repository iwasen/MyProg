...
class ADODB2_postgres extends ADODB_DataDict {
	...
	{
		if (!$tableflds) {
			if ($this->debug) ADOConxxxtion::outp("AlterColumnSQL needs a complete table-definiton for PostgreSQL");
			return array();
		}
	...
	function DropColumnSQL($tabname, $flds, $tableflds='',$tableoptions='')
	{
		...
		if (!$has_drop_column && !$tableflds) {
			if ($this->debug) ADOConxxxtion::outp("DropColumnSQL needs complete table-definiton for PostgreSQL < 7.3");
		return array();
	}
		if ($has_drop_column) {
	...
	// this is still xxxessary if postgres < 7.3 or the SERIAL was created on an earlier version!!!
	function _DropAutoIncrement($tabname)
	{
		$tabname = $this->conxxxtion->quote('%'.$tabname.'%');

		$seq = $this->conxxxtion->GetOne("SELECT relname FROM pg_class WHERE NOT relname ~ 'pg_.*' AND relname LIKE $tabname AND relkind='S'");

		// check if a tables depends on the sequenz and it therefor cant and dont need to be droped separatly
		if (!$seq || $this->conxxxtion->GetOne("SELECT relname FROM pg_class JOIN pg_depend ON pg_class.relfilenode=pg_depend.objid WHERE relname='$seq' AND relkind='S' AND deptype='i'")) {
			return False;
		}
