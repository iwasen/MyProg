...
class ListMemberInputView extends WaiwaiBaseView {
	// ■ビュー実行
	public function execute() {
		...
		// テンプレート設定
		$this->setTemplate('ListMemberInput.html');
		...
		if ($member_id) {
			...
			// 管理者リスト設定
			$this->setAttribute('waiwai_admin_sort', $request->getAttribute('waiwai_admin_sort'));
			$admin_list_ary = $request->getAttribute('waiwai_admin_list_ary');
			$this->setAttribute('waiwai_admin_list_ary', $this->formatList($admin_list_ary));

			// メンバーリスト設定
			$this->setAttribute('waiwai_member_sort', $request->getAttribute('waiwai_member_sort'));
			$member_list_ary = $request->getAttribute('waiwai_member_list_ary');
			$this->setAttribute('waiwai_member_list_ary', $this->formatList($member_list_ary));
	...
	private function formatList($list_ary) {
		...
		if (is_array($list_ary)) {
			...
			foreach ($list_ary as $list) {
				$list['no'] = $no++;
				$list['remark_num'] = number_format($list['remark_num']);
				$list['last_date'] = $this->format_date($list['last_date'], '-');
