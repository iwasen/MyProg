...
// １メール送信
function send_mail_one($subject, $from, $to, $reply_to, $body, $personal) {
	$send_mail_id = send_mail_data($subject, $from, $reply_to, $body, '', '', $personal);
	send_mail_list($send_mail_id, $to);
	return send_mail_ok($send_mail_id);
}

// メールテンプレート取得
function get_mail_template($template_id, &$subject, &$from, &$cc, &$bcc, &$repty_to, &$body, &$footer) {
	$sql = "SELECT * FROM t_mail_template WHERE mtm_template_id='$template_id'";
	$result = db_exec($sql);
