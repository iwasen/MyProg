...
function onclick_cancel(user_id) {
	if (confirm("このユーザの無反応状態を解除します。よろしいですか？"))
...
<table border=0 cellspacing=0 cellpadding=1 width="100%">
	<tr>
		<td class="lt">■1週間以上反応のないユーザのメール一覧</td>
...
?>
	<tr class="tc0">
		<td align="center"><input type="checkbox" name="open[]" value="<?=$comm_id?>" <?=checked(open_checked($comm_id))?> style="height:15" onclick="open_thread(<?=$user_id?>)"><?if ($fetch->cmt_answer_flag == DBTRUE) echo '<br><font color="red"><nobr>【未回答】</nobr></font>'?></td>
		<td align="center"><input type="button" value="解除" style="height:20" onclick="onclick_cancel(<?=$user_id?>)"></td>
