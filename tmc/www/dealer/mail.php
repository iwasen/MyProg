...
// スレッド内容表示
function display_thread($comm_id, $disp_comm_id, &$seq_ary, $depth = ' ') {
	global $g_seq_no;

	if (strlen($depth) > MAX_THREAD_DEPTH)
		return;

	$sql = "SELECT cml_comm_id,cml_user_id,cml_date,cml_send_recv,cmc_subject,cmc_body,cmc_comm_type,ups_name_kanji,stf_name"
			. " FROM t_comm_log"
	...
	if (pg_num_rows($result)) {
		...
		if ($fetch->cml_comm_id != $disp_comm_id) {
?>
...
<tr class="read_main">
	<th colspan="3">本文</th>
	<td colspan="2"><pre><?=htmlspecialchars($body)?></pre></td>
</tr>
<?
		}
	} else
	...
	for ($i = 0; $i < $nrow; $i++) {
		...
		display_thread($fetch->cml_comm_id, $disp_comm_id, $seq_ary, $depth . '└ ');
	}
}
...
// メール件名とスレッド最上位ID取得
$sql = "SELECT cmc_subject,cml_thread_comm_id,cml_user_id"
		. " FROM t_comm_log"
		. " JOIN t_comm_contents ON cmc_contents_id=cml_contents_id"
		. " WHERE cml_comm_id=$comm_id";
$result = db_exec($sql);
...
// ページ遷移履歴設定
set_page_history(3, htmlspecialchars($subject), 'メール内容表示');
?>
...
<tr>
	<td width="50%"><a href="javascript:Open('print.php?comm_id=<?=$comm_id?>')" title="このスレッドの印刷用ページを開く">このスレッドの印刷用ページを開く</a></td>
	<td width="50%" align="right"><a href="top.php" title="トップページへ戻る">トップページへ戻る</a></td>
</tr>
</table>
<br>
...
<table border="0" cellspacing="2" cellpadding="2" align="center" width="100%" id="mail" class="text12" bgcolor="#ffffff">
<tr bgcolor="#f7df3e">
	<th width="5%">SEQ</th>
	<th width="20%">送信者</th>
	<th width="5%">メール種別</th>
...
<? display_thread($thread_comm_id, $comm_id, $seq_ary) ?>
</table>
