========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : MMLib
========================================================================


AppWizard が生成した MMLib には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには MMLib を構成している各ファイルの概要説明が含まれています。

MMLib.h
    このファイルはアプリケーションの中心となるインクルードファイルです。この
    ファイルは他のプロジェクトの固有のインクルードファイル（Resource.h  も含
    みます） をインクルードし、また CMMLibApp アプリケーションクラスを宣
    言します。

MMLib.cpp
    このファイルは CMMLibApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\MMLib.ico
    ...
    ルです。このアイコンはリソース ファイル MMLib.rc によってインクルード
    されます。
...
MMLibDoc.h, MMLibDoc.cpp - ドキュメント
    これらのファイルは CMMLibDoc クラスを含みます。特別なドキュメント デ
    ータを付け加えたり、CMMLibDoc::Serialize を使ったファイルのセーブや
    ロード機能をインプリメントしたいときはこのファイルを編集してください。

MMLibView.h, MMLibView.cpp - ビュー
    これらのファイルは CMMLibView クラスを含みます。
    CMMLibView オブジェクトは CMMLibDoc オブジェクトを表示するために
    使います。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) MMLib.pch 
