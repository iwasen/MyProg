...
sub1 = "SELECT MC_ct_id,SUM(count) AS MC_count,COUNT(*) AS MC_count_uniq" & vbCrLf & _
			"FROM (SELECT MC_ct_id,COUNT(*) AS count FROM T_MAIL_CLICK GROUP BY MC_ct_id,MC_member_id)" & vbCrLf & _
			"GROUP BY MC_ct_id"
...
End If
response.write send_count & " " & ctu_total
%>
<html>
<head>
...
For Each s In body
	...
	Else
		For i = 1 To n
			p = InStr(s, url1(i))
			If p > 0 Then
				s = Replace(s, url1(i), "<a href='" & url2(i) & "' target='_blank'>" & url1(i) & "</font>")
...
%>
	...
	<table width="25%" border="0" cellspacing="5" cellpadding="0">
		<tr>
			<td nowrap class="np">※CTR：クリック数（重複あり）／配信数</td>
		</tr>
		<tr>
			<td nowrap class="np">※CTR(U)：クリックした人数（重複なし）／配信数</td>
		</tr>
	</table>
	<p>
