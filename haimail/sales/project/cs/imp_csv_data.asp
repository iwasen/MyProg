...
sql = "SELECT RM_seq_no,RM_recv_date,MM_member_id,MM_seibetsu,MM_seinengappi,MM_mikikon,SG_shokugyou_name,ZC_todoufuken,ZC_shikuchouson,AN_question_no,AN_answer_text,AS_answer_no" & vbCrLf & _
			...
			"WHERE RM_mail_id='" & mail_id & "' AND RM_imp_valid='1' AND RM_seq_no=AN_seq_no AND AN_question_type='I' AND AN_seq_no=AS_seq_no(+) AND AN_question_type=AS_question_type(+) AND AN_question_no=AS_question_no(+) AND RM_member_id=MM_member_id AND MM_zip_kyojuu=ZC_zip_uniq(+) AND MM_shokugyou=SG_shokugyou_code(+)" & vbCrLf & _
