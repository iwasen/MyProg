...
' Content:全配信感想文内容画面処理
'******************************************************

'=== メイン処理 ===
Dim id, sql, ds, ds2, body, ary, s, flag

SetHeaderTitle "全配信 感想文レスポンス表示"
...
sql = "SELECT * FROM T_MAIL_READ" & vbCrLf & _
			"WHERE MR_login_type='" & g_login_type & "' AND MR_login_id='" & g_login_id & "' AND MR_enq_imp='I' AND MR_recv_mail=" & id
Set ds2 = CreateDynaset(sql, ORADYN_DEFAULT)
If ds2.EOF Then
	ds2.AddNew
	ds2("MR_login_type").Value = g_login_type
	ds2("MR_login_id").Value = g_login_id
	ds2("MR_enq_imp").Value = "I"
