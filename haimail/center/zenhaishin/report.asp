...
<%
'******************************************************
' System :はいめーる・net事務局ページ
' Content:はいめーる通信実施報告書作成処理
'******************************************************
...
Private Sub WriteExcel(job_id, tempfile)
	...
	Dim path, mail_id, row, col, count, i, n, click, resp, ary, item, item2, nendai
	Dim total_send, total_click, total_resp, sum, cur_date, nSheet

	path = Server.MapPath("excel/haimail_tsuushin.xlt")
	...
	With objWorkBook.Sheets(2)
		...
		sql = "SELECT *" & vbCrLf & _
					"FROM T_ZENHAISHIN,T_SEND_MAIL" & vbCrLf & _
					"WHERE ZH_job_id=" & job_id & " AND ZH_mail_id=SM_mail_id(+)"
		...
		If Not ds.EOF Then
			...
			.Cells(row + 2, col + 2).Value = ds("SM_real_send").Value
			.Cells(row + 3, col + 2).Value = ds("SM_point_jushin").Value
			.Cells(row + 4, col + 2).Value = ds("SM_point_enquete").Value
			.Cells(row + 5, col + 2).Value = ds("SM_point_kansou").Value
			.Cells(row + 6, col + 2).Value = Year(ds("SM_limit_date").Value)
			.Cells(row + 6, col + 4).Value = Month(ds("SM_limit_date").Value)
			.Cells(row + 6, col + 6).Value = Day(ds("SM_limit_date").Value)
			.Cells(row + 6, col + 8).Value = Hour(ds("SM_limit_date").Value)
...
Private Sub OutputExcel(tempfile)
	...
	Response.AddHeader "Content-Disposition", "inline; filename=""はいめーる通信実施報告書.xls"""
