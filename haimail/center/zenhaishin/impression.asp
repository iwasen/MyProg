...
' Content:全配信感想文レスポンスリスト表示画面
'******************************************************

'=== メイン処理 ===
Dim job_id

SetHeaderTitle "全配信 感想文レスポンスリスト表示"
...
function OnClick_text() {
	document.form1.action = "imp_text.asp";
	document.form1.submit();
}
function OnClick_list() {
	document.form1.action = "imp_list.asp";
	document.form1.submit();
}
function OnClick_csv() {
	document.form1.action = "imp_csv.asp";
	document.form1.submit();
}
function OnClick_report() {
	document.form1.action = "imp_report.asp";
	document.form1.submit();
}
function OnClick_clear() {
	if (confirm("既読のメールを全て未読に設定します。よろしいですか？"))
		location.href = "impression2.asp?job_id=<%=job_id%>"
}
//-->
</SCRIPT>
</head>
...
<form method="post" name="form1">
	...
	<table border=0 cellspacing=0 cellpadding=0 width='80%'>
		<tr>
			<td class="m0">■感想文レスポンスリスト表示条件</td>
		</tr>
		<tr>
			<td>
	...
	<input type="button" name="report" value="レポート生成" onclick="window.open('report.asp?job_id=<%=job_id%>&file=imp.dat')">
