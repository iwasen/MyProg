...
'=== ÉÅÉCÉìèàóù ===
SetHeaderTitle "10ÅDèTï î}ëÃÅ~ÇoÇqñºï "
Response.ContentType = "application/vnd.ms-excel"

Dim sql, sub1, sub2, sub3, sub4, ds, i, click_total, kari_total, hon_total, baitai_id, where
...
Do While cur_date <= end_date And Not ds.EOF
	...
	Writeln "<th></th>"
	Writeln "<th>PRñºèÃ</th>"
	Writeln "<th>çLçê¿≤Ãﬂ</th>"
	Writeln "<th>∏ÿØ∏êî</th>"
	Writeln "<th>âºìoò^</th>"
	...
	Do While Not ds.EOF
		...
		If row_count > 0 Then
			If DateAdd("ww", 1, cur_week) > end_week Then
				Writeln "<td rowspan=" & row_count & " align='right' valign='top'>" & Day(cur_week) & "(" & WeekDayName(Weekday(cur_week),True) &") Å` " & Day(end_week) &  "(" & WeekDayName(Weekday(end_week),true) &")</td>"
			Else
				Writeln "<td rowspan=" & row_count & " align='right' valign='top'>" & Day(cur_week) & "(" & WeekDayName(Weekday(cur_week),True) &") Å` " & Day(DateAdd("d", 7-WeekDay(cur_week), cur_week)) &  "(" & WeekDayName(Weekday(DateAdd("d", 7-WeekDay(cur_week), cur_week)),true) &")</td>"
			End If
		End If
