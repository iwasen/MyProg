...
SetHeaderTitle "９．媒体別累計"
Response.ContentType = "application/vnd.ms-excel"

Dim sql, ds, ds1, ds2, ds3, bt_id
...
sql = "SELECT PR_baitai_id AS CL_baitai,COUNT(*) AS CL_count FROM T_CLICK_LOG, T_PR" & vbCrLf & _
			"WHERE PR_baitai_id<>'03' AND PR_pr_id=CL_pr_id(+) AND TO_CHAR(CL_date,'YYYYMM') BETWEEN '" & start_ym & "' AND '" & end_ym & "'" & vbCrLf & _
			"GROUP BY PR_baitai_id" & vbCrLf & _
			"ORDER BY CL_baitai"
Set ds1 = CreateDynaset(sql, ORADYN_READONLY)

sql = "SELECT PR_baitai_id AS KT_baitai,COUNT(*) AS KT_count FROM T_KARI_TOUROKU, T_PR" & vbCrLf & _
...
sql = "SELECT PR_baitai_id AS MM_baitai,COUNT(*) AS MM_count FROM T_MEMBER, T_PR" & vbCrLf & _
...
sql = "SELECT * FROM T_BAITAI ORDER BY BT_baitai_id"
...
Do Until ds.EOF
	bt_id = ds("BT_baitai_id").value

	Writeln "<tr>"
	Writeln "<td align='center'>" & ds("BT_name").value & "</td>"

	count1 = 0
	If bt_id = ds1(0).Value Then
