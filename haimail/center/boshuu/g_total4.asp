...
'=== メイン処理 ===
SetHeaderTitle "４．週別広告タイプ別"
Response.ContentType = "application/vnd.ms-excel"

Dim sub1, sql, ds, ds1, ds2, ds3, ds4, i, click_total, kari_total, hon_total, koukoku_id, where
...
Do While cur_date <= end_date
	...
	Writeln "■" & Year(cur_date) & "年" & Month(cur_date) & "月　　広告タイプ＝" & GetKoukokuTypeName(koukoku_id) & ""
	...
	Do While cur_week <= end_week
		If DateAdd("ww", 1, cur_week) > end_week Then
			Writeln "<td align='right'>" & Day(cur_week) & "(" & WeekDayName(Weekday(cur_week),true) &") ～ " & Day(end_week) &  "(" & WeekDayName(Weekday(end_week),true) &")</td>"
		Else
			Writeln "<td align='right'>" & Day(cur_week) & "(" & WeekDayName(Weekday(cur_week),true) &") ～ " & Day(DateAdd("d", 7-WeekDay(cur_week), cur_week)) &  "(" & WeekDayName(Weekday(DateAdd("d", 7-WeekDay(cur_week), cur_week)),true) &")</td>"
		End If
