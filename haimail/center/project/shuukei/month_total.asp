...
sub3 = "SELECT DA_A.DA_dairiten_id DA_dairiten_id,DA_A.DA_name DA_Aname,DA_A.DA_class DA_class,DA_A.DA_rep_id DA_rep_id,DA_B.DA_name DA_Bname FROM T_DAIRITEN DA_A,T_DAIRITEN DA_B WHERE DA_A.DA_rep_id=DA_B.DA_dairiten_id(+)"

sql = "SELECT PJ_project_id,PJ_project_name,JB_job_id,DA_Aname,DA_class,DA_Bname,CM_name,TS_name1,JB_job_name,JB_mail_type,SM_point_jushin,SM_point_enquete,SM_point_kansou,SM_status,SM_real_send AS SR_count,RM_enq_count,RM_imp_count,CC_count" & vbCrLf & _
			"FROM (" & sub1 & "),(" & sub2 & "),(" & sub3 & "),T_COMPANY,T_TANTOUSHA,T_SEND_MAIL,T_JOB,T_PROJECT" & vbCrLf & _
			"WHERE TO_CHAR(SM_start_date,'YYYYMM') = '" & s_ym & "' AND JB_mail_id=SM_mail_id(+) AND PJ_project_id=JB_project_id(+)" & vbCrLf & _
			"AND JB_mail_id=RM_mail_id(+) AND JB_job_id=CC_job_id(+)" & vbCrLf & _
			"AND PJ_tantousha_id=TS_tantousha_id AND TS_company_id=CM_company_id　AND (SM_real_send IS NOT NULL)" & vbCrLf & _
...
Do Until ds.EOF
	...
	Writeln "<td align='right'>" & PercentForm_0(RM_enq_count, SR_count) & "<br></td>"
...
If Not SR_total = 0 Then
	Writeln "<td colspan=8 align='center'>計</td>"
