...
//メイン処理
//kim_start seq no=68 文字変更
set_global('Proアンケート代理実施', BACK_TOP);
//kim_end seq no=68
...
function onclick_marketer(marketer_name, marketer_id) {
	...
	<!--kim_start seq no=68文字変更 //-->
	if (confirm(marketer_name + "さんのIDでログインし、Proアンケートを代理実施します。よろしいですか？")) {
	<!-- kim_end seq no=68 //-->
...
?>
				<tr class="tc<?=$i % 2?>">
					...
					<td align="left"><a href='JavaScript:onclick_marketer("<?="$fetch->mr_name1 $fetch->mr_name2"?>","<?=$fetch->mr_marketer_id?>")' title="Proアンケートの代理実施を実行します"><?=htmlspecialchars("$fetch->mr_name1 $fetch->mr_name2")?></a></td>
