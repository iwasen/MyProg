...
?>
								<td width=15 height=<?=floor($cnt[$tno] / $member_cnt * 120)?> bgcolor="#CC3333"></td>
<?
					} else
						echo "<td><br></td>\n";
?>
...
?>
								<td width="550">
<?
					if ($question->fa_flag == 't' && $sel_no == $question->fa_sno || $question->question_type == 3) {
?>
								<a href="mkk_resdet_fa.php?enquete_id=<?=$enquete_id?>&question_no=<?=$question_no?>&sel_no=<?=$sel_no?>&type=age" target="_blank" title="新しいウィンドウで定性回答を表示します"><?=$sno?>．<?=htmlspecialchars($sel_text)?></a>
<?
					} else {
?>
