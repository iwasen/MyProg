...
<table width="788" border="0" cellspacing="0" cellpadding="0" bgcolor="#ffffff">
	...
	<tr>
		<td>			<table width="600" border="1" cellspacing="0" cellpadding="5" bordercolor="#c0c0c0" bordercolorlight="#c0c0c0" bordercolordark="#c0c0c0">
				<tr>
...
<?
	...
	for ($i = 0; $i < $nrow; $i++) {
		$fetch = pg_fetch_object($result, $i);
		$age_text_ary[$fetch->ge_age_cd] = str_replace('才', '才<br>', $fetch->ge_age_text);
	}
...
?>
					<td width="40" align="center" rowspan="<?=$age_cnt?>"><?=$sex_text?></td>
<?
						}
?>
					<td width="40" align="center" nowrap><?=$age_text_ary[$age]?></td>
<?
						for ($sel_no = 1; $sel_no <= MAX_PRO_SEL; $sel_no++) {
							if ($question->hyoutou[$sel_no]) {
?>
					<td width="40" align="center"><?=number_format($cnt[$sex][$age][$sel_no])?><br><?=format_percent($cnt[$sex][$age][$sel_no], $num[$sex][$age])?></td>
<?
							} else {
?>
					<td><br></td>
...
?>
					<td width="40" align="center"><?=number_format($num[$sex][$age])?><br>100%</td>
				</tr>
<?
					}
				}
...
?>
								<a href="pe_resdet_fa.php?pro_enq_no=<?=$pro_enq_no?>&enquete_id=<?=$enquete_id?>&question_no=<?=$question_no?>&sel_no=<?=$sel_no?>&type=sex_age" target="_blank" title="新しいウィンドウで定性回答を表示します"><?=$sno?>．<?=htmlspecialchars($sel_text)?></a>
<?
					} else {
?>
...
?>
									...
									<input type="hidden" name="type" value="sex_age">
