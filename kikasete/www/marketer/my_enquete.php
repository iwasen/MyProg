...
define('MAX_ENQ', 3);		// アンケート質問最大数
define('MAX_SEL', 10);	// 選択肢最大数
...
class my_enquete_class {
	...
	function write_db() {
		...
		if ($this->my_enq_no == '') {
			...
			$sql = "INSERT INTO t_my_enquete (me_marketer_id,me_my_enq_no,me_make_step,me_enquete_id,me_search_id,me_send_num,me_mail_title,me_mail_header,me_mail_contents,me_mail_footer,me_creator_id,me_ans_cond) VALUES ($this->marketer_id,$this->my_enq_no,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)";
		} else {
			$sql = "UPDATE t_my_enquete SET me_make_step=%s,me_enquete_id=%s,me_search_id=%s,me_send_num=%s,me_mail_title=%s,me_mail_header=%s,me_mail_contents=%s,me_mail_footer=%s,me_creator_id=%s,me_ans_cond=%s WHERE me_marketer_id=$this->marketer_id AND me_my_enq_no=$this->my_enq_no";
		}
		$sql = sprintf($sql,
...
function check_myenq1(&$myenq, &$msg) {
	...
	if ($enquete->point == '')
		$msg[] = 'ポイント数を入力してください。';
	elseif (!check_num($enquete->point, 0, 99))
		$msg[] = 'ポイント数は0〜99を入力してください。';
