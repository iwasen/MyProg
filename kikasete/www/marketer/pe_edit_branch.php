...
?>
												<input type="button" value="改ページの解除" onclick="if (confirm('改ページを解除します。よろしいですか？')) location.href='pe_update_pb.php?pro_enq_no=<?=$pro_enq_no?>&main=<?=$main?>&qno=<?=$qno?>&pb=0'"<?=isset($enquete->branch_cond[$qno]) ? ' disabled' : ''?>>
<?
			} else {
?>
												<input type="button" value="改ページの設定" onclick="location.href='pe_update_pb.php?pro_enq_no=<?=$pro_enq_no?>&main=<?=$main?>&qno=<?=$qno?>&pb=1'"<?=isset($enquete->branch_cond[$qno]) ? ' disabled' : ''?>>
<?
			}
?>
												<input type="button" value="分岐条件の設定" onclick="location.href='pe_edit_branch2.php?pro_enq_no=<?=$pro_enq_no?>&main=<?=$main?>&qno=<?=$qno?>'"<?=$branch_button_flag ? '' : ' disabled'?>>
<?
		}
?>
											</td>
...
?>
			...
			<br>
			<input type="button" value="プレビュー" onclick="location.href='pe_webprev.php?pro_enq_no=<?=$pro_enq_no?>&main=<?=$main?>'">
