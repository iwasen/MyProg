...
*/
$sql = "SELECT me_enquete_id as id, en_status as status, en_end_date as date, me_send_num as send_num"
		. " FROM t_enquete"
		. " INNER JOIN t_my_enquete ON en_enquete_id = me_enquete_id"
		. " WHERE t_enquete.en_end_date BETWEEN " . sql_date(date('Y/m/d', $st)) . " AND " . sql_date(date('Y/m/d', $ed))
		. " ORDER BY en_end_date,me_enquete_id";
...
while ($d <= $ed) {
	...
	if ($i < $nrow) {
		for ( ; $i < $nrow; $i++) {
			...
			$job .= "<tr>\n";
			$job .= "<td align='center'><a href='../job/job_show.php?job_id=$fetch->id' title='ジョブ登録内容を表示します'><font color='$color'>$fetch->id</font></a></td>\n";
