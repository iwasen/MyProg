...
if ($my_enq_no) {
	$sql2 = "SELECT me_enquete_id,me_real_send FROM t_my_enquete WHERE me_marketer_id=$marketer_id AND me_my_enq_no=$my_enq_no";
	$result2 = db_exec($sql2);
	if (pg_numrows($result2)) {
		$fetch2 = pg_fetch_object($result2, 0);
		$send_num2 = number_format($fetch2->me_real_send);
		$enquete_id2 = $fetch2->me_enquete_id;
	} else {
	$send_num2 = 0;
	}
}
