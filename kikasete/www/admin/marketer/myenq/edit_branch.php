...
?>
									<input type="button" value="改ページの解除" onclick="if (confirm('改ページを解除します。よろしいですか？')) location.href='update_pb.php?marketer_id=<?=$marketer_id?>&my_enq_no=<?=$my_enq_no?>&qno=<?=$qno?>&pb=0'"<?=isset($enquete->branch_cond[$qno]) ? ' disabled' : ''?>>
<?
			} else {
?>
									<input type="button" value="改ページの設定" onclick="location.href='update_pb.php?marketer_id=<?=$marketer_id?>&my_enq_no=<?=$my_enq_no?>&qno=<?=$qno?>&pb=1'"<?=isset($enquete->branch_cond[$qno]) ? ' disabled' : ''?>>
<?
			}
?>
									<input type="button" value="分岐条件の設定" onclick="location.href='edit_branch2.php?marketer_id=<?=$marketer_id?>&my_enq_no=<?=$my_enq_no?>&qno=<?=$qno?>'"<?=$branch_button_flag ? '' : ' disabled'?>>
<?
		}
?>
								</td>
...
<br>
<input type="button" value="プレビュー" onclick="location.href='web_prev.php?marketer_id=<?=$marketer_id?>&my_enq_no=<?=$my_enq_no?>'">
