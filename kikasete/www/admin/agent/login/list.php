...
//メイン処理
set_global('agent', 'エージェント管理', 'ログイン履歴', BACK_TOP);
...
// ソート条件
$order_by = order_by(1, 1, 'lg_date', 'lg_login_id', 'lg_member_id', 'ag_name1_kana||ag_name2_kana', 'lg_ip_addr');

// 表示行数条件
$limit = disp_limit();

$sql = "SELECT lg_date,lg_login_id,lg_member_id,ag_name1,ag_name2,lg_ip_addr"
		. " FROM l_login LEFT JOIN t_agent ON lg_member_id=ag_agent_id"
...
?>
	<tr class="tc<?=$i % 2?>">
		...
		<td><a href="<?=$top?>/common/agent_info.php?mail_addr=<?=$fetch->lg_login_id?>" target="_blank" title="エージェント情報を表示します"><?=htmlspecialchars($fetch->lg_login_id)?></a></td>
