...
set_global2('mypartner', 'Ｍｙパートナー管理', '発信数集計', BACK_TOP);
...
while ($d <= $ed) {
	...
	if ($i < $nrow) {
		for ( ; $i < $nrow; $i++) {
			...
			$sql = "SELECT * FROM t_mp_pjt WHERE ppj_enquete_id = $fetch->id";
			$result2 = db_exec($sql);
			...
			$job .= "<tr>\n";
			$job .= "<td align='center'><a href='../pjt/pjt_detail.php?pjt_id={$enq_fetch->ppj_pjt_id}'>$fetch->id</a></td>\n";
