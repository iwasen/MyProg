...
?>
				<tr>
					<td class="m1" rowspan=2>条件<?=$cno?></td>
						...
						<table border="1" cellspacing="0" cellpadding="0" class="n1">
							...
							<tr>
								<td>
									...
									<input type="radio" name="sel_and_or[<?=$cno?>]" <?=value_checked('F', $and_or)?> onclick="disp_desc()" <?=$sa_flag ? 'disabled' : ''?>>完全一致
								</td>
						</table>
						<br>
<?
...
<input type="button" value="　戻る　" onclick="location.href='edit_branch.php?marketer_id=<?=$marketer_id?>&pro_enq_no=<?=$pro_enq_no?>&main=<?=$main?>'">
