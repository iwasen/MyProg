...
if (!isset($add_row) && !isset($add_col)) {
	...
	for ($i = 0; $i < $nrow; $i++) {
		...
		$question_num[$i] = $fetch->pfp_question_num;
		$col_ary[$fetch->pfp_question_num] = $i;
	}
	...
	for ($i = 0; $i < $nrow; $i++) {
		$fetch = pg_fetch_object($result, $i);
		$sample_num[$i] = $fetch->pfp_sample_num;
		$row_ary[$fetch->pfp_sample_num] = $i;
	}
	...
	for ($i = 0; $i < $nrow; $i++) {
		...
		$price[$row_ary[$fetch->pfp_sample_num]][$col_ary[$fetch->pfp_question_num]] = $fetch->pfp_price;
	}
}
...
function onsubmit_form1(f) {
	...
	case "update":
		if (!check_data(f))
			return false;
		return confirm("ファインディング料金表を更新します。よろしいですか？");
	}
	return true;
}
...
<?
	if ($row == 0) {
?>
		<td class="tch" align="center" rowspan="<?=$row_num?>"><b>サ<br>ン<br>プ<br>ル<br>数</b></td>
<?
	}
?>
...
<br>
<input type="submit" value="サンプル数追加" onclick="document.form1.next_action.value='add_row';action='edit.php?add_row=1'">
<input type="submit" value="質問数追加" onclick="document.form1.next_action.value='add_col';action='edit.php?add_col=1'">
<input type="submit" value="　更新　" onclick="document.form1.next_action.value='update';action='update.php'">
