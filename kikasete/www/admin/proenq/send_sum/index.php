...
set_global2('proenquete', 'Ｐｒｏアンケート管理', 'スケジュール', BACK_TOP);
...
while ($d <= $ed) {
	...
	if ($i < $nrow) {
		for (; $i < $nrow; $i++) {
			...
			$sql = "SELECT * FROM t_pro_enquete WHERE mep_enquete_id = $fetch->id OR mep_enquete2_id = $fetch->id";
			$result2 = db_exec($sql);
			...
			$job .= "<td align='center'><a href='../manage/show.php?marketer_id={$enq_fetch->mep_marketer_id}&pro_enq_no={$enq_fetch->mep_pro_enq_no}'>$fetch->id</font></td>\n";
