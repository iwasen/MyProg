...
function get_enq_description(&$myp) {
	...
	// 午前ならAMを付加
	$recruit_end_h = $myp->recruit_end_h;
	if ($recruit_end_h < 12)
		$recruit_end_h = "AM$recruit_end_h";
	...
	$template = str_replace('%LIMIT_DATE%', mb_convert_kana($myp->recruit_end_y, 'N') . '年' . mb_convert_kana($myp->recruit_end_m, 'N') . '月' . mb_convert_kana($myp->recruit_end_d, 'N') . '日（' . $week . '）' . mb_convert_kana($recruit_end_h, 'A') . '：００', $template);
...
function get_enq_body(&$myp, &$header, &$body, &$footer) {
	...
	if (pg_numrows($result)) {
		...
		$header = str_replace('%LIMIT_DATE%', "{$myp->recruit_end_y}年{$myp->recruit_end_m}月{$myp->recruit_end_d}日{$recruit_end_h}時", $header);
		...
		$body = str_replace('%LIMIT_DATE%', mb_convert_kana("{$myp->recruit_end_m}月{$myp->recruit_end_d}日（" . get_week(strtotime("$myp->recruit_end_y/$myp->recruit_end_m/$myp->recruit_end_d")) . "）{$recruit_end_h}時", 'A'), $body);
