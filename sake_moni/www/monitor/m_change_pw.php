...
// 入力パラメータ取得
$old_password = get_password($_POST['old_password']);
$new_password = get_password($_POST['new_password']);
$conf_password = get_password($_POST['conf_password']);
$monitor_id = $_SESSION['ss_monitor_id'];

if ($_POST['edit']) {
	if ($old_password == '')
		$err_old_password = '旧パスワードが入力されていないようです';
	else {
		$sql = "SELECT mn_password FROM t_monitor WHERE mn_monitor_id=$monitor_id";
		if (db_fetch1($sql) != $old_password)
			$err_old_password = '旧パスワードが正しくないようです';
	}

	if ($new_password == '')
		$err_new_password = '新パスワードが入力されていないようです';

	if ($conf_password == '')
		$err_conf_password = '新パスワード確認が入力されていないようです';
	elseif ($new_password != $conf_password)
		$err_conf_password = '新パスワードと新パスワード確認が一致していないようです';

	if ($err_old_password == '' && $err_new_password == '' && $err_conf_password == '') {
		$change_pw['old_password'] = $old_password;
		$change_pw['new_password'] = $new_password;
		$_SESSION['change_pw'] = $change_pw;

		redirect('m_change_pw_conf.php');
	}
}
?>
<HTML>
...
<!--▼パスワード変更フォーム▼-->
<form method="post" action="m_change_pw.php">
		...
		<TD bgcolor=#FFFFFF>
			...
			<TD>
				...
				<TD align="center" valign="center">
					...
					<TR>
					<TD><IMG SRC="images/monitor00_20.gif" WIDTH=102 HEIGHT=22 ALT="旧パスワード"></TD>
					<TD><span class="t12"><INPUT size=30 name="old_password" type="password" <?=value($old_password)?>></span></TD>
					</TR>
<?
if ($err_old_password != '') {
?>
					<TR>
					<TD></TD>
					<TD><span class="err">＜<?=$err_old_password?>＞</span></TD>
					</TR>
<?
}
?>
					<TR>
					<TD><IMG SRC="images/monitor00_21.gif" WIDTH=102 HEIGHT=22 ALT="新パスワード"></TD>
					<TD><span class="t12"><INPUT size=30 name="new_password" type="password" <?=value($new_password)?>></span></TD>
					</TR>
<?
if ($err_new_password != '') {
?>
					<TR>
					<TD></TD>
					<TD><span class="err">＜<?=$err_new_password?>＞</span></TD>
					</TR>
<?
}
?>
					<TR>
					<TD><IMG SRC="images/monitor00_22.gif" WIDTH=102 HEIGHT=22 ALT="新パスワード確認"></TD>
					<TD><span class="t12"><INPUT size=30 name="conf_password" type="password" <?=value($conf_password)?>></span></TD>
					</TR>
<?
...
?>
					<TR>
					<TD></TD>
					<TD><span class="err">＜<?=$err_conf_password?>＞</span></TD>
					</TR>
<?
}
?>
