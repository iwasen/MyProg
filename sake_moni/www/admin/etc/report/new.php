...
function onSubmit_form1(f) {
	...
	if (f.report_file.value == "") {
		alert("レポートファイルを入力してください。");
		f.report_file.focus();
		return false;
	}
	if (f.sum_file.value == "") {
		alert("集計結果ファイルを入力してください。");
		f.sum_file.focus();
		return false;
	}
	if (f.enquete_file.value == "") {
		alert("調査票ファイルを入力してください。");
...
<table border=0 cellspacing=2 cellpadding=3 width="80%">
	...
	<tr>
		<td class="m1" width="20%"><input type="radio" name="chain_or_gyoutai" value="1" checked>チェーン<?=MUST_ITEM?></td>
		<td class="n1" width="80%"><select name="chain_cd"><? select_chain('- 選択してください -', '') ?></select></td>
	</tr>
	<tr>
		<td class="m1" width="20%"><input type="radio" name="chain_or_gyoutai" value="2">業態別<?=MUST_ITEM?></td>
		<td class="n1" width="80%"><select name="gyoutai_cd"><? select_gyoutai('- 選択してください -', '') ?></select></td>
	</tr>
	<tr>
