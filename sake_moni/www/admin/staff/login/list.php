...
//メイン処理
set_global('staff', 'スタッフ管理', 'ログイン履歴', BACK_TOP);
...
// ソート条件
$order_by = order_by(1, 1, 'lg_date', 'lg_login_id', 'lg_member_id', 'st_name1_kana||st_name2_kana', 'lg_ip_addr');

// 表示行数条件
$limit = disp_limit();

$sql = "SELECT lg_date,lg_login_id,lg_member_id,st_name1,st_name2,lg_ip_addr"
		. " FROM l_login LEFT JOIN t_staff ON lg_member_id=st_staff_id"
...
?>
	<tr class="tc<?=$i % 2?>">
		...
		<td><a href="<?=$top?>/common/staff_info.php?mail_addr=<?=$fetch->lg_login_id?>" target="_blank" title="スタッフ情報を表示します"><?=htmlspecialchars($fetch->lg_login_id)?></a></td>
