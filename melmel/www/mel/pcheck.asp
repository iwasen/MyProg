...
Else
	...
	SQL = "SELECT MM_point FROM T_MELMEL WHERE MM_mel_id = '" & Session("mel_id") & "'"
...
<!------------------ 本文textboxテーブル・ここから ------------------------------>
      <TABLE width="461" border="0" cellspacing="5" cellpadding="0">
        <TR>
          <TD>
            <P align="center"><IMG src="images/t_pcheck.gif" width="144" height="23" alt="ポイント精算確認"><BR>
              <IMG name="chara02" src="../images/chara02.gif" width="82" height="55" border="0" alt="キャラクターイメージ">
            </P>
            <P align="center"><FONT size="3" class="kihon">
            <% if (Request.Form("ssn") <> "") then %>
            <% ' if (Point >= 3000) then %>
				以下の通り、ポイントを精算してよろしいでしょうか？</FONT>
			<% else  %>
				精算ポイントまで到達していません。
			<% end if  %>
            </P>
...
'=================== FOR DEBUG MODE ==================='
                  if (Request.Form("ssn") <> "") then
			           Response.Write ("<FORM name=""form1"" METHOD=""POST"" ACTION=""ssn.asp"">")
					   Response.Write ("<INPUT TYPE=""hidden"" NAME=""ssn"" VALUE=""")
...
'=================== FOR DEBUG MODE ==================='
                  if (Request.Form("ssn") <> "") then
	                   Response.Write( FormatNumber(Point - (Int(Request.Form("ssn"))),0))
	              Else
	                   Response.Write(FormatNumber(Round(Point,0),0))
				  End if
	              %>
                  ポイント</FONT></TD>
                </TR>
...
<% '=================== FOR DEBUG MODE ===================' %>
            <% if (Request.Form("ssn") <> "") then %>
				...
				<INPUT type="hidden" name="zen" VALUE="<% Response.Write(Point) %>">
